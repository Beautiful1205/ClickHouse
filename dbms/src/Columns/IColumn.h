...
/// Declares interface to store columns in memory.
    ...
    public:
        ...
        /// 此列存储一个常量值. 这只适用于ColumnConst wrapper
        ...
        virtual StringRef getRawData() const {
            throw Exception("Column " + getName() + " is not a contiguous block of memory",
                            ErrorCodes::NOT_IMPLEMENTED);
        }

        /// If valuesHaveFixedSize, returns size of value, otherwise throw an exception.
        virtual size_t sizeOfValueIfFixed() const {
            throw Exception("Values of column " + getName() + " are not fixed size.",
    ...
    protected:
        ...
        std::vector<MutablePtr> scatterImpl(ColumnIndex num_columns, const Selector &selector) const {
            ...
            if (num_rows != selector.size())
                throw Exception(
                        "Size of selector: " + std::to_string(selector.size()) + " doesn't match size of column: " +
    ...
    using MutableColumns = std::vector<MutableColumnPtr>;//可变的列
