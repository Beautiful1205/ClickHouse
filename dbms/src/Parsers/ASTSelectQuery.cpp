...
#undef CLONE
    ...
    void ASTSelectQuery::formatImpl(const FormatSettings &s, FormatState &state, FormatStateStacked frame) const {
        ...
        s.ostr << (s.hilite ? hilite_keyword : "") << indent_str << "SELECT " << (distinct ? "DISTINCT " : "")
        ...
        if (group_by_with_rollup)
            s.ostr << (s.hilite ? hilite_keyword : "") << s.nl_or_ws << indent_str << (s.one_line ? "" : "    ")
                   << "WITH ROLLUP" << (s.hilite ? hilite_none : "");

        if (group_by_with_cube)
            s.ostr << (s.hilite ? hilite_keyword : "") << s.nl_or_ws << indent_str << (s.one_line ? "" : "    ")
                   << "WITH CUBE" << (s.hilite ? hilite_none : "");

        if (group_by_with_totals)
            s.ostr << (s.hilite ? hilite_keyword : "") << s.nl_or_ws << indent_str << (s.one_line ? "" : "    ")
                   << "WITH TOTALS" << (s.hilite ? hilite_none : "");
        ...
        for (const auto &child : tables_in_select_query.children) {
            ...
            if (tables_element.array_join) {
                if (!array_join)
                    ...
                    throw Exception("Support for more than one ARRAY JOIN in query is not implemented",
            ...
            if (tables_element.table_join) {
                if (!joined_table)
                    ...
                    throw Exception("Multiple JOIN disabled or does not support the query. "
                                    "'set allow_experimental_multiple_joins_emulation' to enable.",
                                    ErrorCodes::NOT_IMPLEMENTED);
            }
        }
