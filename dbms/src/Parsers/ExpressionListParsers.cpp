...
namespace DB {
    ...
    bool ParserList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        ...
        auto list = std::make_shared<ASTExpressionList>();//list数据结构
        node = list;

        while (true) {
            if (first) {//第一个元素走这个逻辑进行处理
                ASTPtr elem;
                if (!elem_parser->parse(pos, elem, expected))//解析第一个元素
                    break;

                list->children.push_back(elem);//解析完成, 向语法树添加元素的操作
                first = false;
            } else {//第二个及后面的元素走这个逻辑进行处理
                auto prev_pos = pos;

                if (!separator_parser->ignore(pos, expected))//解析关键字(分隔符)
                    break;

                ASTPtr elem;
                if (!elem_parser->parse(pos, elem, expected)) {//解析第二、三、四...个元素
                    pos = prev_pos;
                    break;
                }
    ...
    static bool parseOperator(IParser::Pos &pos, const char *op, Expected &expected) {
        if (isWordCharASCII(*op)) {//如果op是关键字, 则按关键字进行解析
            return ParserKeyword(op).ignore(pos, expected);
        } else {//如果不是关键字, 则判断两个条件.(在内存中比较op和pos指向的token的是否相同)
    ...
    //解析左结合的二元中缀表达式
    //根据相应的操作符operator, 将其转化成对应的函数ASTFunction
    bool ParserLeftAssociativeBinaryOperatorList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        ...
        while (1) {
            if (first) {
                ...
                const char **it;
                for (it = operators; *it; it += 2)//遍历operators数组, 看当前的token是哪个操作符. 为什么+=2呢？具体要看operators[], 每行一个是符号, 一个是解释
                ...
                /** special exception for the access operator to the element of the array `x[y]`,
                 * which contains the infix part '[' and the suffix ''] '(specified as' [')
                  */
                if (0 == strcmp(it[0], "[")) {
    ...
    //OR 和 AND都是走的这套逻辑.
    // infix = "OR", function_name = "or"
    // 或
    // infix = "AND", function_name = "and"
    ...
    bool ParserBetweenExpression::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        ...
        //(subject BETWEEN left AND right)   转化成   (subject >= left AND subject <= right)
        //(subject NOT BETWEEN left AND right)   转化成   (subject < left OR subject > right)
    ...
    bool ParserLambdaExpression::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        ParserToken arrow(TokenType::Arrow);//Arrow箭头
        ...
        do {
            ...
            if (!ParserList(std::make_unique<ParserIdentifier>(),
                            std::make_unique<ParserToken>(TokenType::Comma)).parse(pos, inner_arguments, expected))
                break;
    ...
    bool ParserPrefixUnaryOperatorExpression::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        ...
        /// Let's parse chains of the form `NOT NOT x`. This is hack. 不支持`NOT NOT x`
        ...
        if (it[0] && 0 == strncmp(it[0], "NOT", 3)) {
            /// Was there an even number of NOTs.  NOT的个数是不是偶数？
            bool even = false;
    ...
    bool ParserOrderByExpressionList::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        return ParserList(std::make_unique<ParserOrderByElement>(), std::make_unique<ParserToken>(TokenType::Comma),
                          false)
                .parse(pos, node, expected);
    }
