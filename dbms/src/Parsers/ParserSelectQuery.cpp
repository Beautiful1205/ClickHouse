...
namespace DB {
    ...
    bool ParserSelectQuery::parseImpl(Pos &pos, ASTPtr &node, Expected &expected) {
        ...
        ParserNotEmptyExpressionList exp_list_for_with_clause(false,
                                                              true); /// Set prefer_alias_to_column_name for each alias.   WITH子句中, 为每个别名设置prefer_alias_to_column_name=true
        //带select子句的表达式
        ParserNotEmptyExpressionList exp_list_for_select_clause(
                true);    /// Allows aliases without AS keyword.           SELECT语句中允许使用别名时不使用AS关键字, 如select a b from table where a <> '', a的别名是b

        ParserExpressionWithOptionalAlias exp_elem(false); // 解析可能使用到别名的表达式, 如select a AS b from table where a <> ''

        ParserOrderByExpressionList order_list;//order子句
        ...
        /// WITH expr list
        {
            if (s_with.ignore(pos, expected)) {//检测当前解析的token是否是关键字WITH, 如果是关键字WITH, 则继续解析with子句
                if (!exp_list_for_with_clause.parse(pos, with_expression_list, expected))//如果解析with子句失败, 则表示SQL有问题, 返回false, 结束解析过程
                    return false;
            }
        }
        ...
        if (s_group_by.ignore(pos, expected)) {
            ...
            if ((select_query->group_by_with_rollup || select_query->group_by_with_cube) &&
