  ...
  */
    ...
    public:
        ...
        struct FormatSettings {
            ...
            char nl_or_ws;//next_line or white_space
        ...
        virtual void
        formatImpl(const FormatSettings & /*settings*/, FormatState & /*state*/, FormatStateStacked /*frame*/) const {
            throw Exception("Unknown element in AST: " + getID(), ErrorCodes::UNKNOWN_ELEMENT_IN_AST);
        }

        void cloneChildren();

        void dumpTree(std::string str = "") const
        {
            std::stringstream log_str;
            log_str << "AST:\n";
            dumpTree(log_str);
            LOG_DEBUG(&Logger::get("AST"), str << log_str.str());
        }
