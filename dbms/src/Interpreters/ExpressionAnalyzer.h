...
/// ExpressionAnalyzer sources, intermediates and results.          ExpressionAnalyzer的起始、中间和最终结果数据
/// It splits data and logic, allows to test them separately.       切分数据和逻辑, 允许进行单独测试
/// If you are not writing a test you probably don't need it. Use ExpressionAnalyzer itself. 如果不做测试, 不需要使用它. 只使用ExpressionAnalyzer即可
    struct ExpressionAnalyzerData {
        ...
        NamesAndTypesList aggregated_columns;//完成聚合操作后的所有列
        NamesAndTypesList array_join_columns;//完成ARRAY JOIN操作后的所有列
  ...
  */
            ...
            ExtractedSettings(const Settings &settings)
                    : max_ast_depth(settings.max_ast_depth),
                      ...
                      size_limits_for_set(settings.max_rows_in_set, settings.max_bytes_in_set,
                                          settings.set_overflow_mode),
                      size_limits_for_join(settings.max_rows_in_join, settings.max_bytes_in_join,
    ...
    public:
        ...
        /** These methods allow you to build a chain of transformations over a block, that receives values in the desired sections of the query.
          * 下面这些方法需要在block上构建一系列的操作, 对block中的数据进行这一系列的操作后, 得到最终的结果.
          *
          * Example usage:
