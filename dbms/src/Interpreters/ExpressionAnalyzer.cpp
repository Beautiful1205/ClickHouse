...
namespace DB {
    ...
    ExpressionAnalyzer::ExpressionAnalyzer(
            ...
            : ExpressionAnalyzerData(syntax_analyzer_result_->source_columns, required_result_columns_, subqueries_for_sets_), query(query_), context(context_),
              settings(context.getSettings()), subquery_depth(subquery_depth_), do_global(do_global_), syntax(syntax_analyzer_result_) {
        storage = syntax->storage;
        rewrite_subqueries = syntax->rewrite_subqueries;

        //如果查询的基础表有额外的列, 需要添加到source_columns中, 并做去重处理
        //这里应该是additional_source_columns.empty()=true, 不用走这段逻辑
        if (!additional_source_columns.empty()) {
            source_columns.insert(source_columns.end(), additional_source_columns.begin(),
                                  additional_source_columns.end());
            removeDuplicateColumns(source_columns);
        }

        /// Delete the unnecessary from `source_columns` list. Form `columns_added_by_join`.
        collectUsedColumns();//检查哪些列是需要读取出来的, 删掉不必要的列. 生成columns_added_by_join

        /// external_tables, subqueries_for_sets for global subqueries.
        /// Replaces global subqueries with the generated names of temporary tables that will be sent to remote servers.
        // 全局子查询需要用到的external_tables, subqueries_for_sets.
        // 将全局子查询替换成临时表.
        // 临时表将被发送到remote servers上. 用临时表替换全局子查询后的query将在remote servers上执行, 会用到临时表的数据.
        initGlobalSubqueriesAndExternalTables();

        /// has_aggregation, aggregation_keys, aggregate_descriptions, aggregated_columns.
        // analyzeAggregation()方法应该在initGlobalSubqueriesAndExternalTables()方法之后执行, 原因如下:
        /// This analysis should be performed after processing global subqueries,
        /// because otherwise, if the aggregate function contains a global subquery,
        /// then `analyzeAggregation` method will save in `aggregate_descriptions` the information about the parameters of this aggregate function, among which global subquery.
        /// Then, when you call `initGlobalSubqueriesAndExternalTables` method,
        /// this the global subquery will be replaced with a temporary table,
        /// resulting in aggregate_descriptions will contain out-of-date information,
        /// which will lead to an error when the query is executed.
        // 分析聚合情况, 包括has_aggregation, aggregation_keys, aggregate_descriptions, aggregated_columns等
        analyzeAggregation();
    }
    ...
    void ExpressionAnalyzer::analyzeAggregation() {
         ...
         */
        /** 分析找到1-聚合键、2-有关聚合函数的信息(聚合描述)、3-聚合后或者聚合前完成所有操作后的一组列

          * 下面的所有内容(编译临时表达式操作)-仅用于查询分析(类型输出)
          */

        auto *select_query = query->as<ASTSelectQuery>();

        //包含groupBy子句或having子句, 则表示需要聚合操作
        if (select_query && (select_query->groupBy() || select_query->having()))
            has_aggregation = true;

        //temp_actions 包含了需要对block进行的一系列操作
        ...
        if (select_query) {
            ...
            /// 得到JOIN操作中的右表(注意这里找到的是右表)(节点中包含了JOIN条件)
            const ASTTablesInSelectQueryElement *join = select_query->join();
            if (join) {
                const auto &table_join = join->table_join->as<ASTTableJoin &>();//找到JOIN条件, 判断是使用的USING子句还是ON子句
            ...
            if (select_query->groupBy()) {
                ...
                for (ssize_t i = 0; i < ssize_t(group_asts.size()); ++i) {
                    ...
                    if (!block.has(column_name))
                        throw Exception("Unknown identifier (in GROUP BY): " + column_name,
    ...
    void ExpressionAnalyzer::getRootActions(const ASTPtr &ast, bool no_subqueries, ExpressionActionsPtr &actions,
                                            bool only_consts) {
        LogAST log;
    ...
    void ExpressionAnalyzer::getActionsFromJoinKeys(const ASTTableJoin &table_join, bool no_subqueries,
...
/// "Big" ARRAY JOIN.
    ...
    void ExpressionAnalyzer::addJoinAction(ExpressionActionsPtr &actions, bool only_types) const {
        ...
        if (only_types)
            actions->add(
                    ExpressionAction::ordinaryJoin(nullptr, analyzedJoin().key_names_left, columns_added_by_join_list));
        else
            for (auto &subquery_for_set : subqueries_for_sets)
                if (subquery_for_set.second.join)
                    actions->add(
                            ExpressionAction::ordinaryJoin(subquery_for_set.second.join, analyzedJoin().key_names_left,
                                                           columns_added_by_join_list));
    }

    static void appendRequiredColumns(
            NameSet &required_columns, const Block &sample, const Names &key_names_right,
    ...
    bool ExpressionAnalyzer::appendJoin(ExpressionActionsChain &chain, bool only_types) {
        ...
        if (join_params.strictness == ASTTableJoin::Strictness::Unspecified &&
            join_params.kind != ASTTableJoin::Kind::Cross) {
        ...
        SubqueryForSet &subquery_for_set = subqueries_for_sets[toString(join_hash.first) + "_" +
                                                               toString(join_hash.second)];
        ...
        if (!subquery_for_set.join) {
            ...
            subquery_for_set.join = std::make_shared<Join>(analyzedJoin().key_names_right, settings.join_use_nulls,
                                                           settings.size_limits_for_join, join_params.kind,
    ...
    void ExpressionAnalyzer::getActionsBeforeAggregation(const ASTPtr &ast, ExpressionActionsPtr &actions,
                                                         bool no_subqueries) {
        const auto *node = ast->as<ASTFunction>();
    ...
    void ExpressionAnalyzer::collectUsedColumns() {
        ...
        /// If there are virtual columns among the unknown columns.
        /// Remove them from the list of unknown and add in columns list, so that when further processing they are also considered.
        if (storage) {
