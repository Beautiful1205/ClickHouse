...
namespace DB {
    ...
    static void throwIfNoAccess(Context &context) {
        if (context.getSettingsRef().readonly) {
            ...
            if (client_info.interface == ClientInfo::Interface::HTTP &&
                client_info.http_method == ClientInfo::HTTPMethod::GET)
            ...
            throw Exception("Cannot execute query. DDL queries are prohibited for the user",
                            ErrorCodes::QUERY_IS_PROHIBITED);
    }


    //根据SQL语句是select语句、insert语句还是create语句等等, 选择对应的执行器程序执行
    std::unique_ptr<IInterpreter>
    InterpreterFactory::get(ASTPtr &query, Context &context, QueryProcessingStage::Enum stage) {
        ProfileEvents::increment(ProfileEvents::Query);

        if (query->as<ASTSelectQuery>()) {
            /// This is internal part of ASTSelectWithUnionQuery.
            /// Even if there is SELECT without union, it is represented by ASTSelectWithUnionQuery with single ASTSelectQuery as a child.
            //即使SQL中没有union, 这可以看作是ASTSelectWithUnionQuery内部的一个子节点
