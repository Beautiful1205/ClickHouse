...
namespace DB {
    ...
    static std::unordered_map<String, DataTypePtr>
    requiredRightKeys(const Names &key_names, const NamesAndTypesList &columns_added_by_join) {
        NameSet right_keys;
...
/// Converts column to nullable if needed. No backward convertion.
    ...
    correctNullability(ColumnWithTypeAndName &&column, bool nullable, const ColumnUInt8 &negative_null_map) {
        if (nullable) {
            convertColumnToNullable(column);
    ...
    Join::Type Join::chooseMethod(const ColumnRawPtrs &key_columns, Sizes &key_sizes) {
        ...
        if (keys_size == 1
            && (typeid_cast<const ColumnString *>(key_columns[0])
                || (key_columns[0]->isColumnConst() && typeid_cast<const ColumnString *>(
                &static_cast<const ColumnConst *>(key_columns[0])->getDataColumn()))))
    ...
    template<typename Value, typename Mapped>
    struct KeyGetterForTypeImpl<Join::Type::hashed, Value, Mapped> {
    ...
    namespace {
        ...
        struct Inserter {
            static void
            insert(const Join &, Map &map, KeyGetter &key_getter, Block *stored_block, size_t i, Arena &pool);
        };

        template<typename Map, typename KeyGetter>
        struct Inserter<ASTTableJoin::Strictness::Any, Map, KeyGetter> {
            static ALWAYS_INLINE void
            insert(const Join &join, Map &map, KeyGetter &key_getter, Block *stored_block, size_t i, Arena &pool) {
        ...
        struct Inserter<ASTTableJoin::Strictness::Asof, Map, KeyGetter> {
            static ALWAYS_INLINE void
            insert(Join &join, Map &map, KeyGetter &key_getter, Block *stored_block, size_t i, Arena &pool,
        ...
        void insertFromBlockImplType(
                ...
                const Sizes &key_sizes, Block *stored_block, ConstNullMapPtr null_map, Arena &pool) {
            if (null_map)
                insertFromBlockImplTypeCase<STRICTNESS, KeyGetter, Map, true>(join, map, rows, key_columns, key_sizes,
                                                                              stored_block, null_map, pool);
            else
                insertFromBlockImplTypeCase<STRICTNESS, KeyGetter, Map, false>(join, map, rows, key_columns, key_sizes,
                                                                               stored_block, null_map, pool);
        }
...
#undef M
    ...
    bool Join::insertFromBlock(const Block &block) {
        ...
        for (size_t i = 0; i < keys_size; ++i) {
            materialized_columns.emplace_back(recursiveRemoveLowCardinality(
                    block.getByName(key_names_right[i]).column->convertToFullColumnIfConst()));
            key_columns[i] = materialized_columns.back().get();
        }
    ...
    namespace {
        ...
        void addFoundRow(const typename Map::mapped_type &mapped, AddedColumns &added,
...
#undef M
    ...
    void Join::joinBlockImpl(
        ...
        for (size_t i = 0; i < keys_size; ++i) {
            materialized_columns.emplace_back(recursiveRemoveLowCardinality(
                    block.getByName(key_names_left[i]).column->convertToFullColumnIfConst()));
            key_columns[i] = materialized_columns.back().get();
        }
        ...
        if constexpr (right_or_full) {
            for (size_t i = 0; i < existing_columns; ++i) {
                ...
                if (use_nulls) {
                    if (std::end(key_names_left) ==
                        std::find(key_names_left.begin(), key_names_left.end(), block.getByPosition(i).name))
        ...
        AddedColumns added(sample_block_with_columns_to_add, block_with_columns_to_add, block, blocklist_sample,
                           extras);
    ...
    void Join::checkTypesOfKeys(const Block &block_left, const Names &key_names_left, const Block &block_right) const {
        ...
        for (size_t i = 0; i < keys_size; ++i) {
            ...
            DataTypePtr left_type = removeNullable(
                    recursiveRemoveLowCardinality(block_left.getByName(key_names_left[i]).type));
            DataTypePtr right_type = removeNullable(
                    recursiveRemoveLowCardinality(block_right.getByName(key_names_right[i]).type));
    ...
    void Join::joinGetImpl(Block &block, const String &column_name, const Maps &maps_) const {
        joinBlockImpl<ASTTableJoin::Kind::Left, ASTTableJoin::Strictness::Any>(
                block, {block.getByPosition(0).name}, {}, {sample_block_with_columns_to_add.getByName(column_name)},
                maps_);
    }
...
// TODO: return array of values when strictness == ASTTableJoin::Strictness::All
    ...
    void
    Join::joinBlock(Block &block, const Names &key_names_left, const NamesAndTypesList &columns_added_by_join) const {
        std::shared_lock lock(rwlock);

        checkTypesOfKeys(block, key_names_left, sample_block_with_keys);

        if (dispatch([&](auto kind_, auto strictness_, auto &map) {
            joinBlockImpl<kind_, strictness_>(block, key_names_left, columns_added_by_join,
                                              sample_block_with_columns_to_add, map);
        })) {
            /// Joined
    ...
    protected:
        Block readImpl() override {
            ...
            if (parent.dispatch([&](auto, auto strictness, auto &map) { block = createBlock<strictness>(map); }));
                ...
                throw Exception("Logical error: unknown JOIN strictness (must be on of: ANY, ALL, ASOF)",
                                ErrorCodes::LOGICAL_ERROR);
            return block;
        }
...
#undef M
        ...
        size_t fillColumns(const Map &map, MutableColumns &columns_keys_and_right) {
            ...
            for (; it != end; ++it) {
                ...
                AdderNonJoined<STRICTNESS, typename Map::mapped_type>::add(it->getSecond(), rows_added,
        ...
        getNullabilityChanges(const Block &sample_block_with_keys, const Block &out_block,
