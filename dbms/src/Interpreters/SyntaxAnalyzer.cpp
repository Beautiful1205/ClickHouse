...
/// Add columns from storage to source_columns list.
        void collectSourceColumns(const ASTSelectQuery *select_query, StoragePtr storage,
                                  NamesAndTypesList &source_columns)
        {
            if (storage)
            {
                auto physical_columns = storage->getColumns().getAllPhysical();//实际列
                ...
                if (select_query)
                {
                    const auto &storage_aliases = storage->getColumns().getAliases();//别名列
...
// 1-将限定列名(如db.table.column、table.column、table_alias.column)转换为普通形式的列名
// 2-展开星号和限定星号, 展开成列名
// 3-在转换后, 将有普通形式的列和列别名. 列和列别名将在QueryNormalizer中规范化
        ...
        {
            std::vector<TableWithColumnNames> tables_with_columns = getDatabaseAndTablesWithColumnNames(select_query,
                                                                                                        context);
...
/// Also we have to remove duplicates in case of GLOBAL subqueries. Their results are placed into tables so duplicates are inpossible.
        void
        removeUnneededColumnsFromSelectClause(const ASTSelectQuery *select_query, const Names &required_result_columns,
                                              bool remove_dups)
        {
            ASTs &elements = select_query->select()->children;
        ...
        /// Replacing scalar subqueries with constant values.  用常量值替换标量子查询
...
/// Eliminates injective function calls and constant expressions from group by statement. 从GROUP BY语句中消除内射函数调用和常量表达式
        ...
        void optimizeOrderBy(const ASTSelectQuery *select_query)
        {
            ...
            for (const auto &elem : elems)
            {
                ...
                if (elems_set.emplace(name,
                                      order_by_elem.collation ? order_by_elem.collation->getColumnName() : "").second)
                ...
                if (result.array_join_result_to_source.empty())
                {
                    ...
                    } else /// This is a nested table.
                    {
                        ...
                        for (const auto &column_name : source_columns)
                        {
                            ...
                            {
                                result.array_join_result_to_source[Nested::concatenateName(result_name,
                                                                                           splitted.second)] = column_name;
                                found = true;
                                break;
                            }
                        }
                        if (!found)
                            throw Exception("No columns in nested table " + source_name,
                                            ErrorCodes::EMPTY_NESTED_TABLE);
                    }
                }
            }
        }
...
/// Parse JOIN ON expression and collect ASTs for joined columns.
        void collectJoinedColumnsFromJoinOnExpr(AnalyzedJoin &analyzed_join, const ASTTableJoin &table_join)
        {
            ...
            {
                throw Exception("Invalid expression for JOIN ON. " + msg + supported_syntax,
...
/// Find the columns that are obtained by JOIN.
        ...
        Names qualifyOccupiedNames(NamesAndTypesList &columns, const NameSet &source_columns,
            ...
            StoragePtr storage) const
    {
        ...
        collectSourceColumns(select_query, result.storage, result.source_columns);//收集所有的实际列和别名列
        NameSet source_columns_set = removeDuplicateColumns(result.source_columns);//去除重复的列名
        ...
        if (select_query)
        {
            ...
            translateQualifiedNames(query, *select_query, context,
                                    (storage ? storage->getColumns().getOrdinary().getNames() : source_columns_list),
                                    source_columns_set,
            ...
            //根据distributed_product_mode设置, 针对分布式表查询, 使用visit()方法重写IN和JOIN部分
            InJoinSubqueriesPreprocessor(context).visit(query);

            /// Optimizes logical expressions. 优化逻辑表达式
            LogicalExpressionsOptimizer(select_query,
                                        settings.optimize_min_equality_disjunction_chain_length.value).perform();
        }

        /// Creates a dictionary `aliases`: alias -> ASTPtr
        //构造一个map(字典), 保存alias和ASTPtr之间的映射关系
        {
            LogAST log;
        ...
        /// Common subexpression elimination. Rewrite rules. 消除公共子表达式. 重写规则
        {
        ...
        // 执行标量子查询.(用 常量值 替换 标量子查询) (标量子查询是指子查询返回的结果是一行一列(一个常量/一个常量元组), 如一个数字或一个字符串, 也是子查询中最简单的返回形式)
        executeScalarSubqueries(query, context, subquery_depth);

        /// Optimize if with constant condition after constants was substituted instead of scalar subqueries.
        // 得到标量子查询的返回结果后, 根据得到的常量结果进行if子句的优化
        ...
        if (select_query)
        {
            ...
            /// 用于确定result中的 array_join_alias_to_name, array_join_result_to_source 这两个参数.
            ...
            //分析JOIN情况
            collectJoinedColumns(result.analyzed_join, *select_query, source_columns_set, context.getCurrentDatabase(),
