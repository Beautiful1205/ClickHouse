...
/**
 * 很多种数据结构都可以进行聚合
 * 为了提高效率, 聚合数据本身被放入pool中的
 * 具体的数据和pool的所有权(聚合函数的状态states)保存在convertToBlocks函数中的ColumnAggregateFunction对象中
 *
 * 大多数的数据结构存在于两个版本: normal 和 two-level
 * 比如对于哈希表这种数据结构, two-level哈希表在键的数量较少时效率不高, 但在使用大量不同的键时伸缩性更好, 因为它更便于执行某些并行化的操作（合并、后处理）
 *
 * 为了确保在大部分情况下具有较高的效率, 首先使用单级normal哈希表, 当不同键的数目足够大时, 将它们转换为two-level哈希表
 *
 * ps. 有许多不同的方法可以有效地实现并行和分布式聚合, 适合各种不同的情况. 上面介绍的这种方法只是其中的一种, 综合多种原因选择了这种方法
 */
...
/**具有更好散列函数的变量的bit位数一般都超过了32位
 * 外部聚合的合并阶段, 其中键的数目可能远远大于40亿, 但我们保留在内存中, 同时只合并它们的子分区
 *
 * TODO: 我们需要选用更好的哈希函数, 不仅仅适用于外部聚合, 还适用于在具有兆字节内存的计算机上获得巨大的聚合结果
 */
...
/// For the case where there is one numeric key.
    ...
    struct AggregationMethodOneNumber {
        ...
        static void insertKeyIntoColumns(const typename Data::value_type &value, MutableColumns &key_columns,
                                         const Sizes & /*key_sizes*/) {
            static_cast<ColumnVectorHelper *>(key_columns[0].get())->insertRawData<sizeof(FieldType)>(
                    reinterpret_cast<const char *>(&value.getFirst()));
        }
    };
...
/// For the case where there is one string key.
    ...
    struct AggregationMethodString {
        ...
        static void
        insertKeyIntoColumns(const typename Data::value_type &value, MutableColumns &key_columns, const Sizes &) {
...
/// Single low cardinality column.
    ...
    struct AggregationMethodSingleLowCardinalityColumn : public SingleColumnMethod {
        ...
        static void
        insertKeyIntoColumns(const typename Data::value_type &value, MutableColumns &key_columns_low_cardinality,
                             const Sizes & /*key_sizes*/) {
...
//聚合算子
    struct AggregatedDataVariants : private boost::noncopyable {
        ...
        /** 聚合函数的工作流程(包含中间状态states):
         * 1-聚合时, 使用IAggregateFunction::create创建states
         * 2-可以使用IAggregateFunction::destroy销毁states
         * 3-如果聚合完成, 则Aggregator::convertToBlocks函数中, 指向聚合函数状态states的指针将写入ColumnAggregateFunction.
         *   ColumnAggregateFunction具有states的所有权, 即在其析构函数中调用destroy
         *
         *   如果聚合过程中, 在调用Aggregator::convertToBlocks之前抛出了异常, 则聚合函数的状态states必须被销毁, 以避免内存泄露;
         *   在这种情况下, 要销毁states, 析构函数调用Aggregator::destroyAggregateStates方法, 但前提是变量Aggregator（见下文）不是nullptr;
         *   也就是说, 在将聚合函数状态的所有权转移给ColumnAggregateFunction之前, 需要先设置变量“aggregator”, 这样当发生异常时, states就被正常地销毁了
         *
         * PS. 也可以通过创建一个pool来避免这个问题, 这个pool知道聚合函数的哪些状态和按哪些顺序放置, 并且知道如何销毁它们.
         *     但这很难做到, 因为会把可变长度的字符串放在同一个pool中. 在这种情况下, pool将无法知道存储了哪些偏移对象
         */
        ...
        ///聚合函数中保存中间状态的池. 所有权稍后将转移到ColumnAggregateFunction。
...
#undef M
        ...
        AggregatedDataVariants() : aggregates_pools(1, std::make_shared<Arena>()),
  ...
  */
    ...
    public:
        struct Params {
            ...
            Params(
                    ...
                    : src_header(src_header_),
                      ...
                      overflow_row(overflow_row_), max_rows_to_group_by(max_rows_to_group_by_),
        ...
        /** Merge several aggregation data structures and output the result as a block stream.
          */
        std::unique_ptr<IBlockInputStream>
        mergeAndConvertToBlocks(ManyAggregatedDataVariants &data_variants, bool final, size_t max_threads) const;
    ...
    protected:
        ...
        BlocksList
        prepareBlocksAndFillTwoLevel(AggregatedDataVariants &data_variants, bool final, ThreadPool *thread_pool) const;
