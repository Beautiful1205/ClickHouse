...
/// Default shard weight.
        ...
        inline bool isLocal(const Cluster::Address &address, const Poco::Net::SocketAddress &resolved_address,
            ...
            // 对于有副本的情况，如果满足: 1-port参数和服务器监听的端口一致;2-host对应着一组IP, 且其中一个地址与服务器机器的网络接口地址相同;
            // 则将使用这个分片且不考虑进程间的通信
            // (意思应该是如果满足上面2个条件, 则isLocal=true)
            ///
            /// * - this criteria is somewhat approximate. 这是个近似标准
            ///
            /// Also, replica is considered non-local, if it has default database set (only reason is to avoid query rewrite).
            //此外, 如果副本具有默认数据库集, 则将其视为非本地副本(唯一的原因是避免查询重写)
...
/// Implementation of Cluster::Address class
    ...
    Cluster::Address::Address(const Poco::Util::AbstractConfiguration &config, const String &config_prefix) {
        ...
        secure = config.getBool(config_prefix + ".secure", false) ? Protocol::Secure::Enable
                                                                  : Protocol::Secure::Disable;
        compression = config.getBool(config_prefix + ".compression", true) ? Protocol::Compression::Enable
                                                                           : Protocol::Compression::Disable;
    }


    Cluster::Address::Address(const String &host_port_, const String &user_, const String &password_,
                              UInt16 clickhouse_port, bool secure_)
        ...
        //将host解析成ip
        initially_resolved_address = DNSResolver::instance().resolveAddress(parsed_host_port.first,
    ...
    Cluster::Address Cluster::Address::fromFullString(const String &full_string) {
        ...
        if (!user_pw_end || !colon)
            throw Exception("Incorrect user[:password]@host:port#default_database format " + full_string,
                            ErrorCodes::SYNTAX_ERROR);

        const bool has_pw = colon < user_pw_end;
        const char *host_end = has_pw ? strchr(user_pw_end + 1, ':') : colon;
        if (!host_end)
            throw Exception("Incorrect address '" + full_string + "', it does not contain port",
                            ErrorCodes::SYNTAX_ERROR);
...
/// Implementation of Clusters class
    ...
    Clusters::Clusters(const Poco::Util::AbstractConfiguration &config, const Settings &settings,
    ...
    void Clusters::updateClusters(const Poco::Util::AbstractConfiguration &config, const Settings &settings,
                                  const String &config_name) {
...
/// Implementation of `Cluster` class
    ...
    Cluster::Cluster(const Poco::Util::AbstractConfiguration &config, const Settings &settings,
                     const String &cluster_name) {
        Poco::Util::AbstractConfiguration::Keys config_keys;
        config.keys(cluster_name, config_keys);

        if (config_keys.empty())
            throw Exception("No cluster elements (shard, node) specified in config at path " + cluster_name,
        ...
        for (const auto &key : config_keys) {
            if (startsWith(key, "node")) {
                ...
                ConnectionPoolPtr pool = std::make_shared<ConnectionPool>(
                        ...
                        ConnectionTimeouts::getTCPTimeoutsWithoutFailover(settings).getSaturated(
                ...
                for (const auto &replica_key : replica_keys) {
                    ...
                    if (startsWith(replica_key, "replica")) {
                        ...
                        throw Exception("Unknown element in config: " + replica_key,
                ...
                for (const auto &replica : replica_addresses) {
                    auto replica_pool = std::make_shared<ConnectionPool>(
                            ...
                            ConnectionTimeouts::getTCPTimeoutsWithFailover(settings).getSaturated(
                                    settings.max_execution_time),
                ...
                shards_info.push_back({std::move(dir_name_for_internal_replication), current_shard_num, weight,
                                       std::move(shard_local_addresses), std::move(shard_pool),
                                       std::move(all_replicas_pools), internal_replication});
            } else
        ...
        if (addresses_with_failover.empty())
            throw Exception("There must be either 'node' or 'shard' elements in config",
    ...
    Cluster::Cluster(const Settings &settings, const std::vector<std::vector<String>> &names,
        ...
        for (const auto &shard : names) {
            ...
            shards_info.push_back(
                    {{}, current_shard_num, default_weight, std::move(shard_local_addresses), std::move(shard_pool),
