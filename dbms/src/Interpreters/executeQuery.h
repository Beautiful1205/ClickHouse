...
/// Parse and execute a query.
    void executeQuery(
            ...
            std::function<void(
                    const String &)> set_content_type, /// If non-empty callback is passed, it will be called with the Content-Type of the result.
            std::function<void(
                    const String &)> set_query_id /// If non-empty callback is passed, it will be called with the query id.
    );
...
/// must be done separately.
    BlockIO executeQuery(
            ...
            QueryProcessingStage::Enum stage = QueryProcessingStage::Complete,    /// To which stage the query must be executed. SQL执行是分阶段的, 一般需要执行到complete阶段, 但也有特殊的
