        ...
        if (params.src_header) {
            ...
            for (size_t i = 0; i < params.aggregates_size; ++i) {
                ...
                if (final)
                    ...
                    type = std::make_shared<DataTypeAggregateFunction>(params.aggregates[i].function, argument_types,
        ...
        for (size_t i = 0; i < params.aggregates_size; ++i) {
            ...
            if (i + 1 < params.aggregates_size) {
                ...
                total_size_of_aggregate_states =
                        (total_size_of_aggregate_states + alignment_of_next_state - 1) / alignment_of_next_state *
...
#undef M
        ...
        auto compiler_headers = Poco::Util::Application::instance().config().getString("compiler_headers",
        ...
        for (size_t i = 0; i < params.aggregates_size; ++i) {
            ...
            if (pos == std::string::npos)
                throw Exception(
                        "Cannot compile code: unusual path of header file for aggregate function: " + header_path,
        ...
        aggregate_functions_headers_args << "-include '" << compiler_headers
        ...
        auto get_code = [method_typename_single_level, method_typename_two_level, aggregate_functions_typenames] {
            ...
            auto append_code_for_specialization =
                    [&code, &aggregate_functions_typenames](const std::string &method_typename,
        ...
        auto on_ready = [compiled_data_owned_by_callback](SharedLibraryPtr &lib) {
            ...
            compiled_data_owned_by_callback->compiled_two_level_method_ptr = lib->get<void *(*)()>(
                    "_ZN2DB14getPtrTwoLevelEv")();
        };
    ...
    AggregatedDataVariants::Type Aggregator::chooseAggregationMethod() {
        ...
        for (const auto &pos : params.keys) {
            DataTypePtr type = (params.src_header ? params.src_header : params.intermediate_header).safeGetByPosition(
                    pos).type;
...
/** It's interesting - if you remove `noinline`, then gcc for some reason will inline this function, and the performance decreases (~ 10%).
  ...
  */
    ...
    void NO_INLINE Aggregator::executeImpl(
        ...
        if (!no_more_keys)
            executeImplCase<false>(method, state, aggregates_pool, rows, key_columns, aggregate_instructions, keys,
                                   overflow_row);
        else
            executeImplCase<true>(method, state, aggregates_pool, rows, key_columns, aggregate_instructions, keys,
                                  overflow_row);
    }
        ...
        for (size_t i = 0; i < rows; ++i) {
            ...
            if constexpr (!no_more_keys)  /// Insert.
            {
                ...
                if (emplace_result.isInserted()) {
                    ...
                    aggregate_data = aggregates_pool->alignedAlloc(total_size_of_aggregate_states,
    ...
    bool Aggregator::executeOnBlock(const Block &block, AggregatedDataVariants &result,
        ...
        for (size_t i = 0; i < params.aggregates_size; ++i) {
            for (size_t j = 0; j < aggregate_columns[i].size(); ++j) {
                materialized_columns.push_back(block.safeGetByPosition(
                        params.aggregates[i].arguments[j]).column->convertToFullColumnIfConst());
        ...
        if (result.type == AggregatedDataVariants::Type::without_key) {
            ...
            if (compiled_data->compiled_method_ptr) {
                ...
                executeWithoutKeyImpl(result.without_key, rows, aggregate_functions_instructions.data(),
                                      result.aggregates_pool);
        } else {
...
#undef M
        ...
        auto result_size_bytes = current_memory_usage -
                                 memory_usage_before_aggregation;    /// Here all the results in the sum are taken into account, from different threads.
...
#undef M
        ...
        Block block = prepareBlockAndFill(data_variants, final, method.data.impls[bucket].size(),
                                          [bucket, &method, this](
                                              ...
                                              convertToBlockImpl(method, method.data.impls[bucket],
                                                                 ...
                                                                 final_aggregate_columns, final_);
                                          });
    ...
    void Aggregator::execute(const BlockInputStreamPtr &stream, AggregatedDataVariants &result) {
        ...
        LOG_TRACE(log, std::fixed << std::setprecision(3)
                                  << "Aggregated. " << src_rows << " to " << rows << " rows (from "
                                  << src_bytes / 1048576.0 << " MiB)"
                                  << " in " << elapsed_seconds << " sec."
                                  << " (" << src_rows / elapsed_seconds << " rows/sec., "
                                  << src_bytes / elapsed_seconds / 1048576.0 << " MiB/sec.)");
    }
    ...
    Block Aggregator::prepareBlockAndFillWithoutKey(AggregatedDataVariants &data_variants, bool final,
                                                    bool is_overflows) const {
        size_t rows = 1;
            ...
            if (data_variants.type == AggregatedDataVariants::Type::without_key || params.overflow_row) {
                ...
                for (size_t i = 0; i < params.aggregates_size; ++i) {
                    if (!final_)
                        ...
                        aggregate_functions[i]->insertResultInto(data + offsets_of_aggregate_states[i],
                                                                 *final_aggregate_columns[i]);
                }
...
#undef M
    ...
    BlocksList Aggregator::prepareBlocksAndFillTwoLevel(AggregatedDataVariants &data_variants, bool final,
...
#undef M
    ...
    BlocksList
    Aggregator::convertToBlocks(AggregatedDataVariants &data_variants, bool final, size_t max_threads) const {
        if (isCancelled())
            return BlocksList();
        ...
        LOG_TRACE(log, std::fixed << std::setprecision(3)
                                  ...
                                  << " in " << elapsed_seconds << " sec."
                                  << " (" << rows / elapsed_seconds << " rows/sec., "
                                  << bytes / elapsed_seconds / 1048576.0 << " MiB/sec.)");
        ...
        for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num) {
            ...
            for (size_t i = 0; i < params.aggregates_size; ++i)
                aggregate_functions[i]->merge(res_data + offsets_of_aggregate_states[i],
                                              current_data + offsets_of_aggregate_states[i], res->aggregates_pool);
...
/** Combines aggregation states together, turns them into blocks, and outputs streams.
  ...
  */
/**
 * 将聚合状态组合在一起, 将它们转换为blocks, 并输出streams
 * 如果聚合状态是两级的, 则它严格按照“bucket-num”的顺序生成块（这对于分布式处理很重要）
 * 这样做时, 它可以并行处理不同的bucket, 最多使用threads_个线程
 */
    class MergingAndConvertingBlockInputStream : public IBlockInputStream {
    public:
        /** The input is a set of non-empty sets of partially aggregated data, which are all either single-level, or are two-level.
          * 输入参数aggregator_是一组非空的中间聚合数据的集合, 它们要么是single-level的, 要么是two-level的
          */
        MergingAndConvertingBlockInputStream(const Aggregator &aggregator_, ManyAggregatedDataVariants &data_,
                                             bool final_, size_t threads_)
...
#undef M
    ...
    std::unique_ptr<IBlockInputStream> Aggregator::mergeAndConvertToBlocks(
        ...
        if (non_empty_data.size() > 1) {
            ...
            //按降序对states进行排序, 以便合并过程更加高效(因为所有的states都是要合并到第一个states中)
        ...
        ///如果至少有一个选项是两级的, 则将所有选项转换为两级选项
        ///注意-也许在合并之前不转换单个级别的版本会更好, 但是需要在合并结束时将它们单独合并。
        ...
        for (size_t i = 1, size = non_empty_data.size(); i < size; ++i) {
            if (first->type != non_empty_data[i]->type)//non_empty_data中所有元素的type都必须保持一致
            ...
            /** Elements from the remaining sets can be moved to the first data set.
              ...
              */
            //将non_empty_data中的元素按顺序插入到aggregates_pools中
    ...
    void NO_INLINE Aggregator::mergeStreamsImplCase(
        ...
        for (size_t i = 0; i < params.aggregates_size; ++i)
            aggregate_columns[i] = &typeid_cast<const ColumnAggregateFunction &>(
                    *block.safeGetByPosition(params.keys_size + i).column).getData();
    ...
    void
    Aggregator::mergeStream(const BlockInputStreamPtr &stream, AggregatedDataVariants &result, size_t max_threads) {
        if (isCancelled())
            return;
        ...
        LOG_TRACE(log,
                  "Read " << total_input_blocks << " blocks of partially aggregated data, total " << total_input_rows
                          << " rows.");
