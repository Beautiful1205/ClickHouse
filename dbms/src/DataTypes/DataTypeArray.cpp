...
namespace DB {
    ...
    namespace {
        void
        serializeArraySizesPositionIndependent(const IColumn &column, WriteBuffer &ostr, UInt64 offset, UInt64 limit) {
        ...
        void deserializeArraySizesPositionIndependent(IColumn &column, ReadBuffer &istr, UInt64 limit) {
            ...
            while (i < initial_size + limit && !istr.eof()) {
                ...
                readIntBinary(current_size, istr);//读取二进制数据
    ...
    void DataTypeArray::serializeBinaryBulkWithMultipleStreams(
        ...
        if (auto stream = settings.getter(settings.path)) {
            if (settings.position_independent_encoding)
                ...
                DataTypeNumber<ColumnArray::Offset>().serializeBinaryBulk(*column_array.getOffsetsPtr(), *stream,
                                                                          offset, limit);
        }
        ...
        if (limit == 0 || nested_limit)
            nested->serializeBinaryBulkWithMultipleStreams(column_array.getData(), nested_offset, nested_limit,
                                                           settings, state);
        settings.path.pop_back();
    }
    ...
    void DataTypeArray::deserializeBinaryBulkWithMultipleStreams(
        ...
        if (auto stream = settings.getter(settings.path)) {
            if (settings.position_independent_encoding)
                ...
                DataTypeNumber<ColumnArray::Offset>().deserializeBinaryBulk(column_array.getOffsetsColumn(), *stream,
                                                                            limit, 0);
        }
        ...
        settings.avg_value_size_hint = nested_limit ? settings.avg_value_size_hint / nested_limit * offset_values.size()
                                                    : 0;
        ...
        if (!nested_column.empty() && nested_column.size() != last_offset)
            throw Exception("Cannot read all array values: read just " + toString(nested_column.size()) + " of " +
    ...
    void DataTypeArray::serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr,
    ...
    void DataTypeArray::serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                          const FormatSettings &settings) const {
        const ColumnArray &column_array = static_cast<const ColumnArray &>(column);
        const ColumnArray::Offsets &offsets = column_array.getOffsets();
    ...
    void DataTypeArray::serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                         const FormatSettings &settings) const {
        const ColumnArray &column_array = static_cast<const ColumnArray &>(column);
        const ColumnArray::Offsets &offsets = column_array.getOffsets();
    ...
    void DataTypeArray::serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                         const FormatSettings &settings) const {
    ...
    void DataTypeArray::serializeProtobuf(const IColumn &column, size_t row_num, ProtobufWriter &protobuf,
    ...
    void DataTypeArray::deserializeProtobuf(IColumn &column, ProtobufReader &protobuf, bool allow_add_row,
    ...
    static DataTypePtr create(const ASTPtr &arguments) {
        if (!arguments || arguments->children.size() != 1)
            throw Exception("Array data type family must have exactly one argument - type of elements",
