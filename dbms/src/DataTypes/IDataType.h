  ...
  */
    ...
    public:
        ...
        /** Binary serialization for range of values in column - for writing to disk/network, etc.
          ...
          */
        /* 对列中某个范围内的数据进行序列化, 便于写入磁盘/网络等.
         * 序列化时候, 某些数据类型是在多个流中表示的.
         * 示例:
         * *-Arrays表示为所有元素的流和数组大小的流;
         * *-Nullable表示可为空的类型(没有指定默认值代替NULL的), 也是流为空的标志.
         *
         * 不同的流由“path”进行标识. 如果数据类型需要单个流(对于大多数数据类型都是这样), 则流的路径为空. 否则，路径可能包含"array elements", "array sizes"等元素
         *
         * 对于多维数组, path可以有任意的长度.
         * 例如, 对于二维数字数组, 我们至少有三个流:
         * -数组大小;         (顶级数组的大小）
         * -数组元素/数组大小;  (二级(嵌套)数组的大小)
         * -数组元素/数组元素; （最深的元素, 连续放置)
         *
         * IDataType的子类必须重写这些方法: serializeBinaryBulk, deserializeBinaryBulk等等
         *
         * 单个流的...WithMultipleStreams()方法实现将调用serializeBinaryBulk、deserializeBinaryBulk方法
         */
        ...
        virtual void
        deserializeBinaryBulk(IColumn &column, ReadBuffer &istr, size_t limit, double avg_value_size_hint) const;
        ...
        /** Text serialization with escaping but without quoting.
          */
        virtual void
        serializeAsTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const;
        ...
        /** Text serialization as a literal that may be inserted into a query.
          */
        virtual void
        serializeAsTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const;
        ...
        /** Text serialization for the CSV format.
          */
        virtual void
        serializeAsTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const;
        ...
        /** Text serialization for displaying on a terminal or saving into a text file, and the like.
          ...
          */
        virtual void
        serializeAsText(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const;

        /** Text serialization intended for using in JSON format.
          */
        virtual void
        serializeAsTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const;

        virtual void deserializeAsTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &) const;

        /** Text serialization for putting into the XML format.
          */
        virtual void serializeAsTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                        const FormatSettings &settings) const;

        /** Serialize to a protobuf. */
        virtual void serializeProtobuf(const IColumn &column, size_t row_num, ProtobufWriter &protobuf,
                                       size_t &value_index) const = 0;

        virtual void
        deserializeProtobuf(IColumn &column, ProtobufReader &protobuf, bool allow_add_row, bool &row_added) const = 0;
    ...
    public: // used somewhere in arcadia
        virtual void serializeTextEscaped(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                          const FormatSettings &) const = 0;
    ...
    protected:
        ...
        /** Text serialization as a literal that may be inserted into a query.
          */
        virtual void
        serializeTextQuoted(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const = 0;
        ...
        /** Text serialization for the CSV format.
          */
        virtual void
        serializeTextCSV(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const = 0;
        ...
        /** Text serialization for displaying on a terminal or saving into a text file, and the like.
          ...
          */
        virtual void
        serializeText(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const = 0;

        /** Text serialization intended for using in JSON format.
          * force_quoting_64bit_integers parameter forces to brace UInt64 and Int64 types into quotes.
          */
        virtual void
        serializeTextJSON(const IColumn &column, size_t row_num, WriteBuffer &ostr, const FormatSettings &) const = 0;

        virtual void deserializeTextJSON(IColumn &column, ReadBuffer &istr, const FormatSettings &) const = 0;

        /** Text serialization for putting into the XML format.
          */
        virtual void serializeTextXML(const IColumn &column, size_t row_num, WriteBuffer &ostr,
                                      const FormatSettings &settings) const {
    ...
    public:
        ...
        const IDataTypeCustomTextSerialization *
        getCustomTextSerialization() const { return custom_text_serialization.get(); }
    };
