...
namespace DB {
    ...
    public:
        ...
        template<typename U>
        bool operator>(const DecimalField<U> &r) const { return r < *this; }

        template<typename U>
        bool operator>=(const DecimalField<U> &r) const { return r <= *this; }

        template<typename U>
        bool operator!=(const DecimalField<U> &r) const { return !(*this == r); }
        ...
        T &safeGet() {
            ...
            if (which != requested)
                throw Exception("Bad get: has " + std::string(getTypeName()) + ", requested " +
                                std::string(Types::toString(requested)), ErrorCodes::BAD_GET);
            return get<T>();
        }
...
/// signedness of char is different in Linux on x86 and Linux on ARM.
    ...
    template<>
    struct NearestFieldTypeImpl<UUID> {
        using Type = UInt128;
    };
    template<>
