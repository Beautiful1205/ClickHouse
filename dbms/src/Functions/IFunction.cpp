...
/// It's expected that cache_size is a number of reading streams (so, will store single cached value per thread).
    ...
    ColumnPtr wrapInNullable(const ColumnPtr &src, const Block &block, const ColumnNumbers &args, size_t result,
    ...
    bool PreparedFunctionImpl::defaultImplementationForConstantArguments(Block &block, const ColumnNumbers &args,
                                                                         size_t result,
        ...
        for (size_t arg_num = 0; arg_num < arguments_size; ++arg_num) {
            ...
            if (arguments_to_remain_constants.end() !=
                ...
                temporary_block.insert(
                        {static_cast<const ColumnConst *>(column.column.get())->getDataColumnPtr(), column.type,
                         column.name});
            }
        }
    ...
    bool PreparedFunctionImpl::defaultImplementationForNulls(Block &block, const ColumnNumbers &args, size_t result,
        ...
        if (null_presence.has_nullable) {
            ...
            block.getByPosition(result).column = wrapInNullable(temporary_block.getByPosition(result).column, block,
                                                                args,
    ...
    void
    PreparedFunctionImpl::executeWithoutLowCardinalityColumns(Block &block, const ColumnNumbers &args, size_t result,
                                                              size_t input_rows_count, bool dry_run) {
        ...
        for (auto arg : args) {
            ...
            if (auto *column_const = checkAndGetColumn<ColumnConst>(column.column.get())) {
                ...
                if (can_be_executed_on_default_arguments)
                    ...
                    auto dict_encoded = low_cardinality_column->getMinimalDictionaryEncodedColumn(0,
                                                                                                  low_cardinality_column->size());
    ...
    void PreparedFunctionImpl::execute(Block &block, const ColumnNumbers &args, size_t result, size_t input_rows_count,
        ...
        if (useDefaultImplementationForLowCardinalityColumns()) {
            ...
            if (auto *res_low_cardinality_type = typeid_cast<const DataTypeLowCardinality *>(res.type.get())) {
                ...
                block_without_low_cardinality.safeGetByPosition(
                        result).type = res_low_cardinality_type->getDictionaryType();
                ...
                executeWithoutLowCardinalityColumns(block_without_low_cardinality, args, result,
                                                    block_without_low_cardinality.rows(), dry_run);

                auto keys = block_without_low_cardinality.safeGetByPosition(
                        result).column->convertToFullColumnIfConst();

                auto res_mut_dictionary = DataTypeLowCardinality::createColumnUnique(
                        *res_low_cardinality_type->getDictionaryType());
                ...
                if (indexes) {
                    ...
                    res.column = ColumnLowCardinality::create(res_dictionary, res_indexes->index(*indexes, 0),
                                                              use_cache);
                } else {
    ...
    DataTypePtr FunctionBuilderImpl::getReturnTypeWithoutLowCardinality(const ColumnsWithTypeAndName &arguments) const {
        ...
        if (!arguments.empty() && useDefaultImplementationForNulls()) {
            ...
            if (null_presence.has_nullable) {
                Block nested_block = createBlockWithNestedColumns(Block(arguments), ext::collection_cast<ColumnNumbers>(
                        ext::range(0, arguments.size())));
