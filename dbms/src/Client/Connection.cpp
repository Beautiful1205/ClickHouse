...
namespace DB {
    ...
    void Connection::connect() {
        try {
            ...
            LOG_TRACE(log_wrapper.get(),
                      "Connecting. Database: " << (default_database.empty() ? "(not specified)" : default_database)
                                               << ". User: " << user
                                               << (static_cast<bool>(secure) ? ". Secure" : "")
                                               << (static_cast<bool>(compression) ? "" : ". Uncompressed"));

            if (static_cast<bool>(secure)) {//ssl must be used, secure = 1
#if USE_POCO_NETSSL
            ...
            sendHello();//向服务端发送Hello类型的数据包, 服务端程序*Handler接收数据包并进行处理.
            receiveHello();
    ...
    void Connection::sendHello() {
        /** Disallow control characters in user controlled parameters to mitigate the possibility of SSRF.
          * The user may do server side requests with 'remote' table function.
          * Malicious user with full r/w access to ClickHouse may use 'remote' table function to forge requests
          ...
          */
        /** 禁止在用户控制参数中使用控制字符, 以降低发生SSRF的可能性.(控制字符: 表示某一控制功能的字符, ASCII表中所有输出在32以下的都是控制字符)
          * SSRF(Server-Side Request Forgery: 服务器端请求伪造), 是一种由攻击者构造形成由服务端发起请求的一个安全漏洞
          * 用户可以使用“远程”表功能执行服务器端请求.
          * 对ClickHouse具有完全r/w访问权限的恶意用户可以使用“remote”表功能伪造对网络中除ClickHouse以外的其他服务的请求(例如: SMTP).
          * 在握手连接的用户控制部分限制可能的字符数可以减少这种可能性, 但并不能完全解决这个问题
          */
        ...
        //default_database、user、password中不能包含控制字符
        ...
        writeVarUInt(Protocol::Client::Hello, *out);           //将"0"写到out中
    ...
    void Connection::sendQuery(
        ...
        if (server_revision >= DBMS_MIN_REVISION_WITH_CLIENT_INFO) {
            ...
            if (!client_info || client_info->empty()) {
                ...
                //如果是分布式查询或是带ON CLUSTER <cluster>的SQL, 则SQL将被重写, 设置QueryKind = 2
                //但是具体是怎么重写呢？
                // 对于查询分布式表的SQL, 详见StorageDistributed::read()方法调用的ClusterProxy::executeQuery()方法的实现
                // 对于带ON CLUSTER <cluster>的SQL, 详见DDLWorker.cpp的DDLWorker::tryExecuteQuery方法)
    ...
    void Connection::sendData(const Block &block, const String &name) {
        ...
        //将 block_out(～out)中的信息 写到 Block.
        //注意: 调用的是NativeBlockOutputStream.cpp中重写的write()方法
    ...
    void Connection::sendExternalTablesData(ExternalTablesData &data) {
        ...
        msg << "Sent data for " << data.size() << " external tables, total " << rows << " rows in " << elapsed
            << " sec., "
            << static_cast<size_t>(rows / watch.elapsedSeconds()) << " rows/sec., "
            << maybe_compressed_out_bytes / 1048576.0 << " MiB ("
            << maybe_compressed_out_bytes / 1048576.0 / watch.elapsedSeconds() << " MiB/sec.)";
    ...
    bool Connection::hasReadPendingData() const {
        return last_input_packet_type.has_value() ||
               static_cast<const ReadBufferFromPocoSocket &>(*in).hasPendingData();
    }
