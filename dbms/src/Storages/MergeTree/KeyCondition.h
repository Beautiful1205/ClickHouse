  ...
  */
    ...
    public:
        ...
        Range(const Field &point)
                : left(point), right(point), left_bounded(true), right_bounded(true), left_included(true),
                  right_included(true) {}
...
/** Condition on the index.
  ...
  *
  * Constructs a reverse polish notation from these conditions and can calculate (interpret) its satisfiability over key ranges.
  * 构建一个逆波兰表达式, 以此来确定主键范围
  */
    class KeyCondition {
    public:
    ...
    private:
        ...
        struct RPNElement {
            ...
            Function function = FUNCTION_UNKNOWN;//function默认就是FUNCTION_UNKNOWN, 即可以取任何值Can take any value.
