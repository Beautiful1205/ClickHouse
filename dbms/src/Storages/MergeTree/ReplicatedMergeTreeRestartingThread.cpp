...
/// Used to check whether it's us who set node `is_active`, or not.
    ...
    bool ReplicatedMergeTreeRestartingThread::tryStartup() {
        try {
            ...
            try {
                ...
                LOG_ERROR(log,
                          "Couldn't start replication: " << e.what() << ", " << e.displayText() << ", stack trace:\n"
    ...
    void ReplicatedMergeTreeRestartingThread::removeFailedQuorumParts() {
        ...
        for (auto part_name : failed_parts) {
            auto part = storage.getPartIfExists(
                    part_name, {MergeTreeDataPartState::PreCommitted, MergeTreeDataPartState::Committed,
                                MergeTreeDataPartState::Outdated});

            if (part) {
                LOG_DEBUG(log, "Found part " << part_name
    ...
    void ReplicatedMergeTreeRestartingThread::activateReplica() {
        ...
        ops.emplace_back(
                zkutil::makeCreateRequest(is_active_path, active_node_identifier, zkutil::CreateMode::Ephemeral));
        ...
        try {
            ...
            if (e.code == Coordination::ZNODEEXISTS)
                throw Exception(
                        "Replica " + storage.replica_path + " appears to be already active. If you're sure it's not, "
                                                            "try again in a minute or remove znode " +
                        storage.replica_path + "/is_active manually", ErrorCodes::REPLICA_IS_ALREADY_ACTIVE);
        ...
        storage.replica_is_active_node = zkutil::EphemeralNodeHolder::existing(is_active_path,
                                                                               *storage.current_zookeeper);
    }
