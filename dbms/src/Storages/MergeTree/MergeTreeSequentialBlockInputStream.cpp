...
namespace DB {
    ...
    MergeTreeSequentialBlockInputStream::MergeTreeSequentialBlockInputStream(
            ...
            bool quiet)
            : storage(storage_), data_part(data_part_), part_columns_lock(data_part->columns_lock),
              columns_to_read(columns_to_read_), read_with_direct_io(read_with_direct_io_),
    ...
    try {
        Block res;
        if (!isCancelled() && current_row < data_part->rows_count) {//当查询没有被取消 且 current_row < 当前part的最大row时, 表示还可以从当前的part读取数据
            size_t rows_to_read = data_part->index_granularity.getMarkRows(current_mark);
            bool continue_reading = (current_mark != 0);//是继续读的数据还是第一次读数据？如果是继续读, 就从current_mark开始读取; 如果是第一次读数据, 就跳转到下一个mark处
    ...
    void MergeTreeSequentialBlockInputStream::finish() {
         ...
         * When many sources are created, but simultaneously reading only a few of them, buffers don't waste memory.
         *
         * 关闭文件(在销毁对象之前). 当创建了许多源, 但同时只读取其中的一些源时, 缓冲区不会浪费内存
         */
        reader.reset();
