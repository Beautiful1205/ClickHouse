...
namespace DB {
    ...
    MergeTreeData::MergeTreeData(
        ...
        if (!date_column_name.empty()) {
            try {
                ...
                e.addMessage(
                        "(while initializing MergeTree partition key from date column `" + date_column_name + "`)");
                throw;
            }
        } else {
    ...
    void MergeTreeData::setPrimaryKeyIndicesAndColumns(
        ...
        ASTPtr new_primary_key_expr_list = new_primary_key_ast
                                           ? extractKeyExpressionList(new_primary_key_ast)
                                           : new_sorting_key_expr_list->clone();

        if (merging_params.mode == MergeTreeData::MergingParams::VersionedCollapsing)
            new_sorting_key_expr_list->children.push_back(
        ...
        if (primary_key_size > sorting_key_size)
            throw Exception("Primary key must be a prefix of the sorting key, but its length: "
                            + toString(primary_key_size) + " is greater than the sorting key length: " +
                            toString(sorting_key_size),
                            ErrorCodes::BAD_ARGUMENTS);
        ...
        auto new_sorting_key_expr = ExpressionAnalyzer(new_sorting_key_expr_list, new_sorting_key_syntax,
                                                       global_context)
                .getActions(false);
        ...
        auto new_primary_key_expr = ExpressionAnalyzer(new_primary_key_expr_list, new_primary_key_syntax,
                                                       global_context)
                .getActions(false);
    ...
    void MergeTreeData::initPartitionKey() {
        ...
        {
            auto syntax_result = SyntaxAnalyzer(global_context).analyze(partition_key_expr_list,
                                                                        getColumns().getAllPhysical());
            partition_key_expr = ExpressionAnalyzer(partition_key_expr_list, syntax_result, global_context).getActions(
                    false);
        }
    ...
    namespace {
        ...
        void checkTTLExpression(const ExpressionActionsPtr &ttl_expression, const String &result_column_name) {
            ...
            if (!has_date_column)
                throw Exception("TTL expression should use at least one Date or DateTime column",
    ...
    void MergeTreeData::MergingParams::check(const NamesAndTypesList &columns) const {
        if (!sign_column.empty() && mode != MergingParams::Collapsing && mode != MergingParams::VersionedCollapsing)
            throw Exception(
                    "Sign column for MergeTree cannot be specified in modes except Collapsing or VersionedCollapsing.",
                    ErrorCodes::LOGICAL_ERROR);

        if (!version_column.empty() && mode != MergingParams::Replacing && mode != MergingParams::VersionedCollapsing)
            throw Exception(
                    "Version column for MergeTree cannot be specified in modes except Replacing or VersionedCollapsing.",
                    ErrorCodes::LOGICAL_ERROR);
        ...
        auto check_sign_column = [this, & columns](bool is_optional, const std::string &storage) {
            if (sign_column.empty()) {
                ...
                throw Exception("Logical error: Sign column for storage " + storage + " is empty",
                                ErrorCodes::LOGICAL_ERROR);
            }
            ...
            for (const auto &column : columns) {
                if (column.name == sign_column) {
                    if (!typeid_cast<const DataTypeInt8 *>(column.type.get()))
                        throw Exception(
                                "Sign column (" + sign_column + ") for storage " + storage + " must have type Int8."
                                                                                             " Provided column of type " +
                                column.type->getName() + ".", ErrorCodes::BAD_TYPE_OF_FIELD);
                    miss_column = false;
                    break;
                }
            }
        ...
        auto check_version_column = [this, & columns](bool is_optional, const std::string &storage) {
            if (version_column.empty()) {
                ...
                throw Exception("Logical error: Version column for storage " + storage + " is empty",
                                ErrorCodes::LOGICAL_ERROR);
            }
        ...
        for (const String &file_name : part_file_names) {
            ...
            if (broken) {
                if (part->info.level == 0) {
                    ...
                    LOG_ERROR(log, "Considering to remove broken part " << full_path + file_name
                    ...
                    if (contained_parts >= 2) {
                        LOG_ERROR(log, "Considering to remove broken part " << full_path + file_name
                                                                            << " because it covers at least 2 other parts");
                        broken_parts_to_remove.push_back(part);
                    } else {
...
/// The function works for Arrays and Nullables of the same structure.
            ...
            if (!new_types.count(column.name)) {
                ...
                if (!new_type->equals(*old_type) && (!part || part->hasColumnFiles(column.name))) {
                    ...
                    out_expression->add(ExpressionAction::addColumn(
                            {DataTypeString().createColumnConst(1, new_type_name), std::make_shared<DataTypeString>(),
                             new_type_name_column}));
                ...
                new_types[source_and_expression.first]->enumerateStreams(
                        [&](const IDataType::SubstreamPath &substream_path) {
                            ...
                            if (!substream_path.empty() &&
                                substream_path.back().type == IDataType::Substream::ArraySizes)
                                return;

                            String original_file_name = IDataType::getFileNameForStream(original_column_name,
                                                                                        substream_path);
                            String temporary_file_name = IDataType::getFileNameForStream(temporary_column_name,
                                                                                         substream_path);

                            out_rename_map[temporary_file_name + ".bin"] = original_file_name + ".bin";
                            out_rename_map[temporary_file_name + index_granularity_info.marks_file_extension] =
                                    original_file_name + index_granularity_info.marks_file_extension;
                        }, {});
            }
    ...
    void MergeTreeData::alterDataPart(
        ...
        if (!skip_sanity_checks
            ...
            const bool forbidden_because_of_modify =
                    num_files_to_modify > settings.max_files_to_modify_in_alter_columns;
            ...
            exception_message
                    ...
                    << (forbidden_because_of_modify ? "max_files_to_modify_in_alter_columns"
                                                    : "max_files_to_remove_in_alter_columns")
                    << " parameter in configuration file (current value: "
                    << (forbidden_because_of_modify ? settings.max_files_to_modify_in_alter_columns
                                                    : settings.max_files_to_remove_in_alter_columns)
                    << ")";
            ...
            for (const auto &from_to : rename_map) {
                if (!from_to.second.empty()) {
                    try {
                        ...
                        LOG_WARNING(data_part->storage.log,
                                    "Can't remove " << path + from_to.first << ": " << e.displayText());
                    }
                }
            }
        }
    ...
    MergeTreeData::DataPartsVector MergeTreeData::getActivePartsToReplace(
        ...
        auto it_middle = data_parts_by_state_and_info.lower_bound(
                DataPartStateAndInfo{DataPartState::Committed, new_part_info});
        ...
        while (begin != committed_parts_range.begin()) {
            ...
            if (!new_part_info.contains((*prev)->info)) {
                ...
                if (!new_part_info.isDisjoint((*prev)->info))
                    throw Exception(
                            "Part " + new_part_name + " intersects previous part " + (*prev)->getNameWithState() +
        ...
        while (end != committed_parts_range.end()) {
            if ((*end)->info == new_part_info)
                throw Exception("Unexpected duplicate part " + (*end)->getNameWithState() + ". It is a bug.",
                                ErrorCodes::LOGICAL_ERROR);
    ...
    void MergeTreeData::renameTempPartAndReplace(
        ...
        if (covering_part) {
            LOG_WARNING(log, "Tried to add obsolete part " << part_name << " covered by "
                                                           << covering_part->getNameWithState());
            return;
        }
    ...
    void
    MergeTreeData::removePartsFromWorkingSet(const MergeTreeData::DataPartsVector &remove, bool clear_without_timeout,
                                             DataPartsLock & /*acquired_lock*/) {
    ...
    void MergeTreeData::removePartsFromWorkingSet(const DataPartsVector &remove, bool clear_without_timeout,
        ...
        for (auto &part : remove) {
            if (!data_parts_by_info.count(part->info))
                throw Exception("Part " + part->getNameWithState() + " not found in data_parts",
                                ErrorCodes::LOGICAL_ERROR);
    ...
    MergeTreeData::removePartsInRangeFromWorkingSet(const MergeTreePartInfo &drop_range, bool clear_without_timeout,
        ...
        for (const DataPartPtr &part : partition_range) {
            if (part->info.partition_id != drop_range.partition_id)
                throw Exception("Unexpected partition_id of part " + part->name + ". This is a bug.",
                                ErrorCodes::LOGICAL_ERROR);
            ...
            if (part->info.min_block <= drop_range.max_block && drop_range.max_block < part->info.max_block) {
                ...
                String error =
                        "Unexpected merged part " + part->name + " intersecting drop range " + drop_range.getPartName();
                if (!skip_intersecting_parts)
                    throw Exception(error, ErrorCodes::LOGICAL_ERROR);
    ...
    void
    MergeTreeData::forgetPartAndMoveToDetached(const MergeTreeData::DataPartPtr &part_to_detach, const String &prefix,
                                               bool
                                               restore_covered) {
        LOG_INFO(log, "Renaming " << part_to_detach->relative_path << " to " << prefix << part_to_detach->name
                                  << " and forgiving it.");
    ...
    void MergeTreeData::delayInsertOrThrowIfNeeded(Poco::Event *until) const {
        ...
        if (parts_count_in_total >= settings.max_parts_in_total) {
            ...
            throw Exception("Too many parts (" + toString(parts_count_in_total) +
                            ") in all partitions in total. This indicates wrong choice of partition key. The threshold can be modified with 'max_parts_in_total' setting in <merge_tree> element in config.xml or with per-table setting.",
        ...
        if (parts_count_in_partition >= settings.parts_to_throw_insert) {
            ProfileEvents::increment(ProfileEvents::RejectedInserts);
            throw Exception("Too many parts (" + toString(parts_count_in_partition) +
                            "). Merges are processing significantly slower than inserts.", ErrorCodes::TOO_MANY_PARTS);
        }
        ...
        LOG_INFO(log, "Delaying inserting block by "
                << std::fixed << std::setprecision(4) << delay_milliseconds << " ms. because there are "
                << parts_count_in_partition << " parts");
    ...
    MergeTreeData::DataPartsVector
    MergeTreeData::getDataPartsVectorInPartition(MergeTreeData::DataPartState state, const String &partition_id) {
    ...
    MergeTreeData::DataPartPtr MergeTreeData::getPartIfExists(const MergeTreePartInfo &part_info,
                                                              const MergeTreeData::DataPartStates &valid_states) {
        auto lock = lockParts();
    ...
    MergeTreeData::getPartIfExists(const String &part_name, const MergeTreeData::DataPartStates &valid_states) {
        ...
        if (format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING) {
            ...
            if (partition_lit && partition_lit->value.getType() == Field::Types::String) {
                ...
                if (partition_id.size() != 6 || !std::all_of(partition_id.begin(), partition_id.end(), isNumericASCII))
                    throw Exception(
                            "Invalid partition format: " + partition_id +
    ...
    MergeTreeData::DataPartsVector
    MergeTreeData::getDataPartsVector(const DataPartStates &affordable_states, DataPartStateVector *out_states) const {
        DataPartsVector res;
        DataPartsVector buf;
        {
            //保证这个过程是安全的, 即在这个过程中part不会被修改
            auto lock = lockParts();
    ...
    MergeTreeData::DataPartsVector
    MergeTreeData::getAllDataPartsVector(MergeTreeData::DataPartStateVector *out_states) const {
        DataPartsVector res;
        {
            auto lock = lockParts();
    ...
    MergeTreeData::DataPartsVector MergeTreeData::getDataPartsVector() const {
        //返回目前所有Committed状态的part
        return getDataPartsVector({DataPartState::Committed});
    }

    MergeTreeData::DataPartPtr MergeTreeData::getAnyPartInPartition(
            const String &partition_id, DataPartsLock & /*data_parts_lock*/) {
        auto it = data_parts_by_state_and_info.lower_bound(
                DataPartStateAndPartitionID{DataPartState::Committed, partition_id});

        if (it != data_parts_by_state_and_info.end() && (*it)->state == DataPartState::Committed &&
    ...
    MergeTreeData::DataPartsVector
    MergeTreeData::Transaction::commit(MergeTreeData::DataPartsLock *acquired_parts_lock) {
        ...
        if (!isEmpty()) {
            ...
            for (const DataPartPtr &part : precommitted_parts) {
                DataPartPtr covering_part;
                DataPartsVector covered_parts = data.getActivePartsToReplace(part->info, part->name, covering_part,
                                                                             *owing_parts_lock);
                if (covering_part) {
                    ...
                    part->remove_time.store(0,
                                            std::memory_order_relaxed); /// The part will be removed without waiting for old_parts_lifetime seconds.
    ...
    MergeTreeData &MergeTreeData::checkStructureAndGetMergeTreeData(const StoragePtr &source_table) const {
        ...
        if (!src_data)
            throw Exception(
                    "Table " + table_name + " supports attachPartitionFrom only for MergeTree family of table engines."
    ...
    MergeTreeData::MutableDataPartPtr MergeTreeData::cloneAndLoadDataPart(const MergeTreeData::DataPartPtr &src_part,
        ...
        if (Poco::File(dst_part_absolute_path).exists())
            throw Exception("Part in " + dst_part_absolute_path.toString() + " already exists",
                            ErrorCodes::DIRECTORY_ALREADY_EXISTS);

        LOG_DEBUG(log,
                  "Cloning part " << src_part_absolute_path.toString() << " to " << dst_part_absolute_path.toString());
    ...
    void MergeTreeData::freezePartitionsByMatcher(MatcherFn matcher, const String &with_name, const Context &context) {
        ...
        for (const auto &part : data_parts) {
            ...
            if (!startsWith(part_absolute_path, clickhouse_path))
                throw Exception("Part path " + part_absolute_path + " is not inside " + clickhouse_path,
                                ErrorCodes::LOGICAL_ERROR);
