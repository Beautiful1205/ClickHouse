...
namespace DB {
    ...
    MergeTreeReader::MergeTreeReader(const String &path,
            ...
            : data_part(data_part), avg_value_size_hints(avg_value_size_hints), path(path), columns(columns),
              uncompressed_cache(uncompressed_cache), mark_cache(mark_cache), save_marks_in_cache(save_marks_in_cache),
              storage(storage), all_mark_ranges(all_mark_ranges), aio_threshold(aio_threshold),
    ...
    size_t MergeTreeReader::readRows(size_t from_mark, bool continue_reading, size_t max_rows_to_read, Block &res) {
        ...
        try {
            ...
            /// If append is true, then the value will be equal to nullptr and will be used only to check that the offsets column has been already read.
            //指向offset columns和nested data structure columns的指针是公用的. 如果append=true, 这个指针将为nullptr, 并且仅用于检查offsets column是否已被读取
            OffsetColumns offset_columns;

            for (const NameAndTypePair &it : columns) {
                /// The column is already present in the block so we will append the values to the end.
                bool append = res.has(it.name);//如果block中有这一列, 即append=true, 只需要在后面追加数据即可; 否则append=false, 需要先创建这一列
                if (!append)
                ...
                if (const DataTypeArray *type_arr = typeid_cast<const DataTypeArray *>(it.type.get())) {
                    String name = Nested::extractTableName(it.name);//得到嵌套数据类型最外层列的name
                    ...
                    if (!it_inserted.second)//it_inserted.second=false表示向map中添加元素失败, 即map中已经有这个元素了; it_inserted.second=true 表示向map中添加元素成功
                        read_offsets = false;

                    /// need to create new offsets
                    if (it_inserted.second && !append)//it_inserted.second 且 append=false, 这种情况下表示需要创建新的offsets
            ...
            /// Better diagnostics.
            e.addMessage("(while reading from part " + path + " from mark " + toString(from_mark) +
    ...
    void MergeTreeReader::readData(
        ...
        auto get_stream_getter = [&](bool stream_for_prefix) -> IDataType::InputStreamGetter {
            return [&, stream_for_prefix](const IDataType::SubstreamPath &substream_path) -> ReadBuffer * {
                ...
                if (!with_offsets && substream_path.size() == 1 &&
                    substream_path[0].type == IDataType::Substream::ArraySizes)
                    return nullptr;
            ...
            for (const auto &requested_column : columns) {
                ...
                if (!has_column) {
                    ...
                    if (offset_columns.count(offsets_name)) {
                        ...
                        DataTypePtr nested_type = typeid_cast<const DataTypeArray &>(
                                *column_to_add.type).getNestedType();
                        size_t nested_rows = typeid_cast<const ColumnUInt64 &>(*offsets_column).getData().back();

                        ColumnPtr nested_column = nested_type->createColumnConstWithDefaultValue(
                                nested_rows)->convertToFullColumnIfConst();
                        ...
                        column_to_add.column = column_to_add.type->createColumnConstWithDefaultValue(
                                num_rows)->convertToFullColumnIfConst();
                    }
