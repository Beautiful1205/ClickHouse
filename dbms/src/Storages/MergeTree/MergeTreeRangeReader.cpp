...
namespace DB {
    ...
    MergeTreeRangeReader::DelayedStream::DelayedStream(
            ...
            : current_mark(from_mark), current_offset(0), num_delayed_rows(0), merge_tree_reader(merge_tree_reader),
              index_granularity(&(merge_tree_reader->data_part->index_granularity)), continue_reading(false),
              is_finished(false) {
    }
    ...
    size_t MergeTreeRangeReader::DelayedStream::read(Block &block, size_t from_mark, size_t offset, size_t num_rows) {
        //根据起始标记from_mark确定从哪一行开始读取, 即from_mark前有多少行
        size_t num_rows_before_from_mark = index_granularity->getMarkStartingRow(from_mark);

        /// We already stand accurately in required position, so because stream is lazy, we don't read anything and only increment amount delayed_rows
    ...
    MergeTreeRangeReader::Stream::Stream(
            ...
            : current_mark(from_mark), offset_after_current_mark(0), last_mark(to_mark),
        ...
        if (from_mark >= marks_count)
            throw Exception(
                    "Trying create stream to read from mark №" + toString(current_mark) + " but total marks count is "
                    + toString(marks_count), ErrorCodes::LOGICAL_ERROR);

        if (last_mark > marks_count)
            throw Exception(
                    "Trying create stream to read to mark №" + toString(current_mark) + " but total marks count is "
                    + toString(marks_count), ErrorCodes::LOGICAL_ERROR);
    }
    ...
    size_t MergeTreeRangeReader::Stream::readRows(Block &block, size_t num_rows) {
        //从当前mark和当前mark的偏移开始读取num_rows行数据
    ...
    void MergeTreeRangeReader::Stream::toNextMark() {
        ...
        if (current_mark < total_marks_count)
            ...
            throw Exception("Trying to read from mark " + toString(current_mark) + ", but total marks count " +
                            toString(total_marks_count), ErrorCodes::LOGICAL_ERROR);

        offset_after_current_mark = 0;
    }

    size_t
    MergeTreeRangeReader::Stream::read(Block &block, size_t num_rows, bool skip_remaining_rows_in_current_granule) {
    ...
    size_t
    MergeTreeRangeReader::ReadResult::countZeroTails(const IColumn::Filter &filter_vec, NumRows &zero_tails) const {
        zero_tails.resize(0);
    ...
    void MergeTreeRangeReader::ReadResult::collapseZeroTails(const IColumn::Filter &filter_vec,
                                                             IColumn::Filter &new_filter_vec,
    ...
    MergeTreeRangeReader::MergeTreeRangeReader(
            ...
            : merge_tree_reader(merge_tree_reader),
              index_granularity(&(merge_tree_reader->data_part->index_granularity)), prev_reader(prev_reader),
              prewhere_column_name(prewhere_column_name), ordered_names(ordered_names), alias_actions(alias_actions),
              prewhere_actions(std::move(prewhere_actions)), always_reorder(always_reorder),
              remove_prewhere_column(remove_prewhere_column), last_reader_in_chain(last_reader_in_chain),
              is_initialized(true) {
    }
    ...
    MergeTreeRangeReader::ReadResult MergeTreeRangeReader::read(size_t max_rows, MarkRanges &ranges) {
        ...
        if (prev_reader) // If not nullptr, read from prev_reader firstly.
        {
            ...
            if (block) {
                ...
                if (!read_result.block) {
                    if (auto *filter = read_result.getFilter())
                        num_rows = countBytesInFilter(
                                filter->getData()); /// All columns were removed and filter is not always true.
            ...
            if (read_result.block) {
                ...
                merge_tree_reader->fillMissingColumns(read_result.block, should_reorder,
    ...
    MergeTreeRangeReader::ReadResult MergeTreeRangeReader::startReadingChain(size_t max_rows, MarkRanges &ranges) {
        ...
        /// Stream is lazy.
        /// result.num_added_rows is the number of rows added to block which is not equal to result.num_rows_read until call to stream.finalize().
        /// Also result.num_added_rows may be less than result.num_rows_read if the last granule in range also the last in part (so we have to adjust last granule).
        //流的读取是惰性的.
        //在调用stream.finalize()方法之前, result.num_added_rows 和 result.num_rows_read是不相等的.
        //如果range内的最后一个granule 就是 part内的最后一个granule, result.num_added_rows可能小于result.num_rows_read
        {
