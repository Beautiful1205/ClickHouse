...
/// Used generally for reading number of rows less than index granularity to decrease cache misses for fat blocks.
    ...
    public:
        ...
        private:
            ...
            /// Actual reader of data from disk  实际从磁盘读取数据
        ...
        /// DelayedStream(惰性读取流)的简单包装类, 检查读取范围的界限并在标记之间执行步骤
        class Stream {
        public:
            Stream() = default;
        ...
        /// Statistics after next reading step. 读取数据后的统计数据
        class ReadResult {
        public:
        ...
        private:
            ...
            void
            collapseZeroTails(const IColumn::Filter &filter, IColumn::Filter &new_filter, const NumRows &zero_tails);
