...
namespace DB {
    ...
    private:
        struct KeyTuplePositionMapping {
            KeyTuplePositionMapping(size_t tuple_index_, size_t key_index_) : tuple_index(tuple_index_),
        ...
        struct RPNElement {
            ...
            RPNElement(
                    Function function_ = FUNCTION_UNKNOWN, size_t key_column_ = 0,
                    std::unique_ptr<StringBloomFilter> &&const_bloom_filter_ = nullptr)
        ...
        using AtomMap = std::unordered_map<std::string, bool (*)(RPNElement &out, const Field &value,
                                                                 const MergeTreeBloomFilterIndex &idx)>;
    ...
    public:
        MergeTreeBloomFilterIndex(
                ...
                : IMergeTreeIndex(name_, expr_, columns_, data_types_, header_, granularity_),
                  bloom_filter_size(bloom_filter_size_), bloom_filter_hashes(bloom_filter_hashes_), seed(seed_),
