...
/// Construct a block consisting only of possible values of virtual columns
    ...
    size_t MergeTreeDataSelectExecutor::getApproximateTotalRowsToRead(
            const MergeTreeData::DataPartsVector &parts, const KeyCondition &key_condition,
...
/// Converts sample size to an approximate number of rows (ex. `SAMPLE 1000000`) to relative value (ex. `SAMPLE 0.1`).
    ...
    BlockInputStreams MergeTreeDataSelectExecutor::readFromParts(
        ...
        //查询的时候可以指定从哪个part中查询吗？？？
        Names virt_column_names;//虚拟列名
        Names real_column_names;//真实列名
        ...
        std::multiset<String> part_values = VirtualColumnUtils::extractSingleValueFromBlock<String>(
                virtual_columns_block, "_part");
        ...
        if (data.minmax_idx_expr) {
            ...
            if (settings.force_index_by_date &&
                minmax_idx_condition->alwaysUnknownOrTrue()) {//settings中规定必须使用日期索引但SQL没有日期索引的情况下抛出异常
        ...
        ///  as well as `max_block_number_to_read`.
        {
            ...
            for (const auto &part : prev_parts) {
                ...
                if (max_block_numbers_to_read) {
                    ...
                    if (blocks_iterator == max_block_numbers_to_read->end() ||
                        part->info.max_block > blocks_iterator->second)
                        continue;
                }
        ...
        bool use_sampling =
                relative_sample_size > 0 || (settings.parallel_replicas_count > 1 && data.supportsSampling());
        ...
        if (use_sampling) {
            ...
            if (typeid_cast<const DataTypeUInt64 *>(type.get()))
                ...
                throw Exception("Invalid sampling column type in storage parameters: " + type->getName() +
            ...
            if (no_data || (!has_lower_limit && !has_upper_limit)) {
                ...
                if (has_lower_limit) {
                    if (!key_condition.addCondition(data.sampling_expr_column_name,
                                                    Range::createLeftBounded(lower, true)))
                ...
                if (has_upper_limit) {
                    if (!key_condition.addCondition(data.sampling_expr_column_name,
                                                    Range::createRightBounded(upper, false)))
                        throw Exception("Sampling column not in primary key", ErrorCodes::ILLEGAL_COLUMN);
        ...
        size_t sum_marks = 0;    //多少个mark
        size_t sum_ranges = 0;   //多少个range
        //遍历所有的part
        for (auto &part : parts) {
            RangesInDataPart ranges(part, part_index++);//根据part和part的编号构造RangesInDataPart的对象
            ...
            //代码运行到这里, 对于每一个part, 将SQL中可能用到的索引更新到RangesInDataPart对象中
        ...
        LOG_DEBUG(log,
                  "Selected " << parts.size() << " parts by date, " << parts_with_ranges.size() << " parts by key, "
                              << sum_marks << " marks to read from " << sum_ranges << " ranges");

        //代码运行到这里, 所有的part和part内的索引都已经保存在了parts_with_ranges中

        //如果parts_with_ranges为空, 表示无数据, 直接返回NULL
        ...
        if (select.final()) {//select语句中是否有final关键字
        ...
        if (use_sampling)
            for (auto &stream : res)
                stream = std::make_shared<FilterBlockInputStream>(stream, filter_expression,
    ...
    namespace {
        ...
        size_t roundRowsOrBytesToMarks(
            ...
            if (!granularity_info.is_adaptive) {//mark之间是按照固定行数分隔的, is_adaptive = false;
            ...
            } else {//mark之间是按照固定字节大小分隔的, is_adaptive = true;
    ...
    BlockInputStreams MergeTreeDataSelectExecutor::spreadMarkRangesAmongStreams(
        ...
        const size_t max_marks_to_use_cache = roundRowsOrBytesToMarks(
                settings.merge_tree_max_rows_to_use_cache,//使用缓存时的最大行数, 行数小于设置值则使用未压缩的缓存. 如果请求很大, 则不使用缓存(对于不刷新缓存的大型查询)
        ...
        /// 如果 mark文件的数量 大于 mark文件的最大缓存数量, 则不能使用缓存
        ...
        if (sum_marks > 0 && settings.merge_tree_uniform_read_distribution == 1) {
            ...
            if (sum_marks < num_streams * min_marks_for_concurrent_read && parts.size() < num_streams)
                num_streams = std::max((sum_marks + min_marks_for_concurrent_read - 1) / min_marks_for_concurrent_read,
                                       parts.size());

            MergeTreeReadPoolPtr pool = std::make_shared<MergeTreeReadPool>(
                    num_streams, sum_marks, min_marks_for_concurrent_read, parts, data, prewhere_info, true,
                    column_names, MergeTreeReadPool::BackoffSettings(settings), settings.preferred_block_size_bytes,
                    false);
    ...
    BlockInputStreams MergeTreeDataSelectExecutor::spreadMarkRangesAmongStreamsFinal(
        ...
        switch (data.merging_params.mode) {
            ...
            case MergeTreeData::MergingParams::Aggregating:
                merged = std::make_shared<AggregatingSortedBlockInputStream>(to_merge, sort_description,
                                                                             max_block_size);
                break;
