...
namespace DB {
    ...
    Block MergeTreeBaseSelectBlockInputStream::readFromPart() {
        ...
        const auto current_preferred_block_size_bytes = preferred_block_size_bytes; // 默认preferred_block_size_bytes = 1000000 byte = 1 MB
        ...
        auto estimateNumRows = [current_preferred_block_size_bytes, current_max_block_size_rows,
            ...
            if (current_preferred_max_column_in_block_size_bytes) {
                ...
                auto rows_to_read_for_max_size_column
                        = current_task.size_predictor->estimateNumRowsForMaxSizeColumn(
                                current_preferred_max_column_in_block_size_bytes);
                double filtration_ratio = std::max(min_filtration_ratio,
                                                   1.0 - current_task.size_predictor->filtered_rows_ratio);
            ...
            return index_granularity.countMarksForRows(current_reader.currentMark(), rows_to_read,
                                                       current_reader.numReadRowsInCurrentGranule());
        };
        ...
        if (!virt_column_names.empty()) {
            ...
            for (const auto &virt_column_name : virt_column_names) {
                if (virt_column_name == "_part") {
                    ColumnPtr column;
                    if (rows)
                        column = DataTypeString().createColumnConst(rows,
                                                                    task->data_part->name)->convertToFullColumnIfConst();
                    else
                    ...
                    ColumnPtr column;
                    if (rows)
                        column = DataTypeUInt64().createColumnConst(rows,
                                                                    task->part_index_in_query)->convertToFullColumnIfConst();
                    else
                    ...
                    if (rows)
                        column = DataTypeString().createColumnConst(rows,
                                                                    task->data_part->info.partition_id)->convertToFullColumnIfConst();
                    else
                        column = DataTypeString().createColumn();
    ...
    void
    MergeTreeBaseSelectBlockInputStream::executePrewhereActions(Block &block, const PrewhereInfoPtr &prewhere_info) {
        if (prewhere_info) {
