...
//MergeTreeThreadSelectBlockInputStream的一批工作task
    struct MergeTreeReadTask {
        ...
        /// Used to satisfy preferred_block_size_bytes limitation
        // 默认preferred_block_size_bytes = 1000000 byte = 1 MB
        ...
        MergeTreeReadTask(
                const MergeTreeData::DataPartPtr &data_part, const MarkRanges &mark_ranges,
    ...
    struct MergeTreeBlockSizePredictor {
        MergeTreeBlockSizePredictor(const MergeTreeData::DataPartPtr &data_part_, const Names &columns,
        ...
        inline size_t estimateNumRowsForMaxSizeColumn(size_t bytes_quota) const {
            double max_size_per_row = std::max<double>(std::max<size_t>(max_size_per_row_fixed, 1),
                                                       max_size_per_row_dynamic);
