...
/// Merge tree is used for incremental sorting of data.  增量排序
...
/// Parts are merged in the background according to a heuristic algorithm.   基于启发式算法合并parts
...
/// Essentially it is an index too.                                              对于每个part, 记录最小和最大的日期, 保存在minmax_dt.idx文件中, 它本质上也是索引文件
///
/// Data is partitioned by the value of the partitioning expression.             data根据分区字段进行分区
/// Parts belonging to different partitions are not merged - for the ease of administration (data sync and backup).  为了便于管理(数据同步和备份), 不会合并不在一个partition的part
///
...
/// - MergeTreeDataMergerMutator
    ...
    public:
        ...
        struct LessDataPart {
            ...
            bool operator()(const MergeTreePartInfo &lhs, const PartitionID &rhs) const {
                return lhs.partition_id < rhs.toUnderType();
            }

            bool operator()(const PartitionID &lhs, const MergeTreePartInfo &rhs) const {
                return lhs.toUnderType() < rhs.partition_id;
            }
        };
        ...
        /// * Next, if commit() is called, the parts are added to the active set and the parts that are covered by them are marked Outdated.
        /// If neither commit() nor rollback() was called, the destructor rollbacks the operation.
        //向working set中添加parts需要借助中间辅助对象(临时文件),
        //1-预提交的parts(parts已准备好, 但未添加到active set中)
        //2-调用commit(), 将预提交的parts添加到active set中, 它们所覆盖的部分被标记为过时. 如果没有调用commit()或rollback(), 析构函数将回滚该操作.

        //working set: 所有的parts的集合, 结合part的state理解一下DataPartState
        //active set:  可用的parts的集合, 结合part的state理解一下DataPartState
        ...
        /// An object that stores the names of temporary files created in the part directory during ALTER of its columns.
        ...
        struct IndexGranularityInfo {
            ...
            /// mark之间的行数是不是变化的?
            /// 即如果mark之间是按照固定行数分隔的, is_adaptive = false;
            /// 如果mark之间是按照固定字节大小分隔的, is_adaptive = true;
            bool is_adaptive;
        ...
        ///
        //  如果包含日期字段, 根据日期按月分part(根据日期字段, 将每个月的数据放在一个part中)
        //  如果不包含日期字段, 将按照指定的分区字段进行分区
        ...
        ///     Primary key must be a prefix of the sorting key;                   主键字段必须是排序字段的一部分
        ///     If it is nullptr, then it will be determined from order_by_ast.    如果主键字段为空, 则将排序字段作为主键字段
        ///
        /// require_part_metadata - should checksums.txt and columns.txt exist in the part directory.  checksums.txt和columns.txt是否存在
        /// attach - whether the existing table is attached or the new table is created.               已经存在的表还是需要新建表
        ...
        Names getColumnsRequiredForPartitionKey() const override {
            return (partition_key_expr ? partition_key_expr->getRequiredColumns() : Names{});
        }
        ...
        DataPartsVector
        getDataPartsVector(const DataPartStates &affordable_states, DataPartStateVector *out_states = nullptr) const;
        ...
        DataPartPtr
        getActiveContainingPart(const MergeTreePartInfo &part_info, DataPartState state, DataPartsLock &lock);
        ...
        void renameTempPartAndAdd(MutableDataPartPtr &part, SimpleIncrement *increment = nullptr,
        ...
        void removePartsFromWorkingSet(const DataPartsVector &remove, bool clear_without_timeout,
                                       DataPartsLock *acquired_lock = nullptr);
        ...
        DataPartsVector
        removePartsInRangeFromWorkingSet(const MergeTreePartInfo &drop_range, bool clear_without_timeout,
        ...
        void removePartsFromWorkingSetAndCloneToDetached(const DataPartsVector &parts, bool clear_without_timeout,
                                                         const String &prefix = "");
        ...
        void
        forgetPartAndMoveToDetached(const DataPartPtr &part, const String &prefix = "", bool restore_covered = false);
        ...
        cloneAndLoadDataPart(const MergeTreeData::DataPartPtr &src_part, const String &tmp_part_prefix,
    ...
    protected:
        ...
        void modifyPartState(DataPartIteratorByInfo it, DataPartState state) {
            if (!data_parts_by_state_and_info.modify(data_parts_indexes.project<TagByStateAndInfo>(it),
                                                     getStateModifier(state)))
                throw Exception("Can't modify " + (*it)->getNameWithState(), ErrorCodes::LOGICAL_ERROR);
        }
        ...
        void createConvertExpression(const DataPartPtr &part, const NamesAndTypesList &old_columns,
                                     const NamesAndTypesList &new_columns,
