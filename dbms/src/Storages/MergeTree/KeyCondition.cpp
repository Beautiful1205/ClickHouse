...
/** Calculate expressions, that depend only on constants.
  ...
  */
    Block KeyCondition::getBlockWithConstants(
        ...
        Block result
                {
                        {DataTypeUInt8().createColumnConstWithDefaultValue(
                                1), std::make_shared<DataTypeUInt8>(), "_dummy"}
                };//createColumnConstWithDefaultValue(), 这是新增了一列, 取默认值UInt8？？放到result这个Block中

        const auto expr_for_constant_folding = ExpressionAnalyzer(query, syntax_analyzer_result,
                                                                  context).getConstActions();

        expr_for_constant_folding->execute(result);//对result这个Block执行一系列的操作
    ...
    KeyCondition::KeyCondition(
        ...
        for (size_t i = 0, size = key_column_names.size(); i < size; ++i) {
            std::string name = key_column_names[i];//key_column_names是指底层数据表中的主键列, 即定义了哪些主键, 序号从0开始
            if (!key_columns.count(name))          //     key_columns是指SQL中WHERE中的条件列
                key_columns[name] = i;             //如果 SQL中WHERE中的条件列 有使用到 定义的主键, 用map保存该列列名和用到的第几个主键
        }

        /** Evaluation of expressions that depend only on constants. For the index to be used, if it is written, for example `WHERE Date = toDate(now())`.
          * 只依赖常数的表达式求值时, 如果用到的索引的处理方法
          */
        //带常量的Block
...
/** Computes value of constant expression and its data type.
  ...
  */
    bool KeyCondition::getConstant(const ASTPtr &expr, Block &block_with_constants, Field &out_value,
    ...
    void KeyCondition::traverseAST(const ASTPtr &node, const Context &context, Block &block_with_constants) {
        ...
        if (!atomFromAST(node, context, block_with_constants, element)) {//像是在处理非的逻辑, 没太明白
    ...
    bool KeyCondition::canConstantBeWrappedByMonotonicFunctions(
        ...
        for (const ExpressionAction &a : key_expr->getActions()) {
            ...
            if (a.type == ExpressionAction::Type::APPLY_FUNCTION && action.size() == 1 &&
                a.argument_names[0] == expr_name) {
                if (!a.function_base->hasInformationAboutMonotonicity())
                    return false;

                // Range is irrelevant in this case
                IFunction::Monotonicity monotonicity = a.function_base->getMonotonicityForRange(*out_type, Field(),
                                                                                                Field());
    ...
    bool KeyCondition::isKeyPossiblyWrappedByMonotonicFunctions(
        ...
        if (!isKeyPossiblyWrappedByMonotonicFunctionsImpl(node, out_key_column_num, key_column_type,
                                                          chain_not_tested_for_monotonicity))
            return false;

        for (auto it = chain_not_tested_for_monotonicity.rbegin();
             it != chain_not_tested_for_monotonicity.rend(); ++it) {
    ...
    bool KeyCondition::isKeyPossiblyWrappedByMonotonicFunctionsImpl(
        ...
        if (const auto *func = node->as<ASTFunction>()) {
            ...
            if (!isKeyPossiblyWrappedByMonotonicFunctionsImpl(args[0], out_key_column_num, out_key_column_type,
                                                              out_functions_chain))
                return false;
    ...
    static void castValueToType(const DataTypePtr &desired_type, Field &src_value, const DataTypePtr &src_type,
                                const ASTPtr &node) {
    ...
    bool KeyCondition::atomFromAST(const ASTPtr &node, const Context &context, Block &block_with_constants,
                                   RPNElement &out) {
        ...
        if (const auto *func = node->as<ASTFunction>()) {
            ...
            if (functionIsInOrGlobalInOperator(func->name)
                ...
                is_set_const = true;
                       ...
                       && isKeyPossiblyWrappedByMonotonicFunctions(args[0], context, key_column_num, key_expr_type,
                                                                   chain)) {
                key_arg_pos = 0;
            } else if (getConstant(args[1], block_with_constants, const_value, const_type)
                       && canConstantBeWrappedByMonotonicFunctions(args[0], key_column_num, key_expr_type, const_value,
                                                                   const_type)) {
                key_arg_pos = 0;
                is_constant_transformed = true;
            } else if (getConstant(args[0], block_with_constants, const_value, const_type)
                       && isKeyPossiblyWrappedByMonotonicFunctions(args[1], context, key_column_num, key_expr_type,
                                                                   chain)) {
                key_arg_pos = 1;
            } else if (getConstant(args[0], block_with_constants, const_value, const_type)
                       && canConstantBeWrappedByMonotonicFunctions(args[1], key_column_num, key_expr_type, const_value,
                                                                   const_type)) {
                key_arg_pos = 1;
                is_constant_transformed = true;
            } else
            ...
            bool cast_not_needed =
                    ...
                    || (isNativeNumber(key_expr_type) &&
                        isNativeNumber(const_type)); /// Numbers are accurately compared without cast.

            if (!cast_not_needed)
                castValueToType(key_expr_type, const_value, const_type, node);

            return atom_it->second(out, const_value);
        } else if (getConstant(node, block_with_constants, const_value,
    ...
    String KeyCondition::toString() const {//就是把RPN打印出来
        String res;
...
/** Index is the value of key every `index_granularity` rows.               Index是每“Index granularity”行的key值
  * This value is called a "mark". That is, the index consists of marks.    这个值叫做“标记”, 也就是说, 索引由标记组成
  *
  ...
  *
  * The range of tuples can always be represented as a combination of parallelograms.  元组的范围总是可以表示为平行四边形的组合
  ...
  */
    ...
    static bool forAnyParallelogram(
        ...
        if (left_bounded) {
            parallelogram[prefix_size] = Range(key_left[prefix_size]);
            if (forAnyParallelogram(key_size, key_left, key_right, true, false, parallelogram, prefix_size + 1,
                                    callback))
                return true;
        }
        ...
        if (right_bounded) {
            ...
            if (forAnyParallelogram(key_size, key_left, key_right, false, true, parallelogram, prefix_size + 1,
                                    callback))
                return true;
        }
...
/*      std::cerr << "Parallelogram: ";
    ...
    bool
    KeyCondition::mayBeTrueInParallelogram(const std::vector<Range> &parallelogram, const DataTypes &data_types) const {
