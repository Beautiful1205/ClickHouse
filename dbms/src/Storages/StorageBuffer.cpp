...
namespace DB {
    ...
    StorageBuffer::StorageBuffer(const std::string &name_, const ColumnsDescription &columns_,
                                 Context &context_,
                                 size_t num_shards_, const Thresholds &min_thresholds_,
...
/// Reads from one buffer (from one block) under its mutex.
    ...
    BlockInputStreams StorageBuffer::read(
        ...
        if (!no_destination) {
            ...
            const bool dst_has_same_structure = std::all_of(column_names.begin(), column_names.end(),
                                                            [this, destination](const String &column_name) {
                                                                return destination->hasColumn(column_name) &&
                                                                       destination->getColumn(column_name).type->equals(
                                                                               *getColumn(column_name).type);
                                                            });

            if (dst_has_same_structure) {
                /// The destination table has the same structure of the requested columns and we can simply read blocks from there.
                streams_from_dst = destination->read(column_names, query_info, context, processed_stage, max_block_size,
                                                     num_streams);
            } else {
                ...
                for (const String &column_name : column_names) {
                    if (!destination->hasColumn(column_name)) {
                        LOG_WARNING(log, "Destination table " << backQuoteIfNeed(destination_database) << "."
                                                              << backQuoteIfNeed(destination_table)
                                                              << " doesn't have column " << backQuoteIfNeed(column_name)
                    ...
                    if (!dst_col.type->equals(*col.type)) {
                        ...
                        header_after_adding_defaults.getByName(column_name) = ColumnWithTypeAndName(dst_col.type,
                                                                                                    column_name);
                    }
                }
                ...
                if (columns_intersection.empty()) {
                    ...
                    streams_from_dst = destination->read(columns_intersection, query_info, context, processed_stage,
        ...
        if (processed_stage > QueryProcessingStage::FetchColumns)
            for (auto &stream : streams_from_buffers)
                stream = InterpreterSelectQuery(query_info.query, context, stream,
                                                SelectQueryOptions(processed_stage)).execute().in;
    ...
    public:
        ...
        void write(const Block &block) override {
            ...
            if (!storage.no_destination) {
                destination = storage.global_context.tryGetTable(storage.destination_database,
                                                                 storage.destination_table);
                if (destination.get() == &storage)
                    throw Exception("Destination table is myself. Write will cause infinite loop.",
                                    ErrorCodes::INFINITE_LOOP);
            }
    ...
    void StorageBuffer::startup() {
        if (global_context.getSettingsRef().readonly) {
            LOG_WARNING(log,
                        "Storage " << getName() << " is run with readonly settings, it will not be able to insert data."
...
/** NOTE If you do OPTIMIZE after insertion,
  ...
  */
    bool StorageBuffer::optimize(const ASTPtr & /*query*/, const ASTPtr &partition, bool final, bool deduplicate,
                                 const Context & /*context*/) {
        if (partition)
        ...
        if (deduplicate)
            throw Exception("DEDUPLICATE cannot be specified when optimizing table of type Buffer",
                            ErrorCodes::NOT_IMPLEMENTED);
    ...
    bool StorageBuffer::checkThresholds(const Buffer &buffer, time_t current_time, size_t additional_rows,
    ...
    void StorageBuffer::flushBuffer(Buffer &buffer, bool check_thresholds, bool locked) {
        ...
        LOG_TRACE(log, "Flushing buffer with " << rows << " rows, " << bytes << " bytes, age " << time_passed
                                               << " seconds.");
        ...
        /** For simplicity, buffer is locked during write.
            ...
            */
        try {
            writeBlockToDestination(block_to_write,
                                    global_context.tryGetTable(destination_database, destination_table));
        }
        catch (...) {
    ...
    void StorageBuffer::writeBlockToDestination(const Block &block, StoragePtr table) {
        ...
        if (!table) {
            LOG_ERROR(log, "Destination table " << backQuoteIfNeed(destination_database) << "."
                                                << backQuoteIfNeed(destination_table)
                                                << " doesn't exist. Block of data is discarded.");
            return;
        }
        ...
        /** We will insert columns that are the intersection set of columns of the buffer table and the subordinate table.
          ...
          */
        Block structure_of_destination_table = allow_materialized ? table->getSampleBlock()
                                                                  : table->getSampleBlockNonMaterialized();
        Block block_to_write;
        ...
        if (block_to_write.columns() != block.columns())
            LOG_WARNING(log, "Not all columns from block in buffer exist in destination table "
                    << backQuoteIfNeed(destination_database) << "." << backQuoteIfNeed(destination_table)
                    << ". Some columns are discarded.");
    ...
    void StorageBuffer::alter(const AlterCommands &params, const String &database_name, const String &table_name,
    ...
    void registerStorageBuffer(StorageFactory &factory) {
        ...
        factory.registerStorage("Buffer", [](const StorageFactory::Arguments &args) {
            ...
            UInt64 num_buckets = applyVisitor(FieldVisitorConvertToNumber<UInt64>(),
                                              engine_args[2]->as<ASTLiteral &>().value);

            Int64 min_time = applyVisitor(FieldVisitorConvertToNumber<Int64>(),
                                          engine_args[3]->as<ASTLiteral &>().value);
            Int64 max_time = applyVisitor(FieldVisitorConvertToNumber<Int64>(),
                                          engine_args[4]->as<ASTLiteral &>().value);
            UInt64 min_rows = applyVisitor(FieldVisitorConvertToNumber<UInt64>(),
                                           engine_args[5]->as<ASTLiteral &>().value);
            UInt64 max_rows = applyVisitor(FieldVisitorConvertToNumber<UInt64>(),
                                           engine_args[6]->as<ASTLiteral &>().value);
            UInt64 min_bytes = applyVisitor(FieldVisitorConvertToNumber<UInt64>(),
                                            engine_args[7]->as<ASTLiteral &>().value);
            UInt64 max_bytes = applyVisitor(FieldVisitorConvertToNumber<UInt64>(),
                                            engine_args[8]->as<ASTLiteral &>().value);
