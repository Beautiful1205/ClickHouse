        ...
        UInt64 getMaximumFileNumber(const std::string &dir_path) {
            ...
            for (auto it = begin; it != end; ++it) {
                ...
                if (it->status().type() != boost::filesystem::regular_file ||
                    !endsWith(file_path.filename().string(), ".bin"))
                    continue;
                ...
                try {
                    ...
                    e.addMessage("Unexpected file name " + file_path.filename().string() + " found at " +
                                 file_path.parent_path().string() + ", should have numeric base name.");
                    throw;
                }
...
/// For destruction of std::unique_ptr of type that is incomplete in class definition.
    ...
    static ExpressionActionsPtr
    buildShardingKeyExpression(const ASTPtr &sharding_key, const Context &context, NamesAndTypesList columns,
                               bool project) {
    ...
    StorageDistributed::StorageDistributed(
            ...
            : IStorage{columns_}, table_name(table_name),
              ...
              global_context(context_), cluster_name(global_context.getMacros()->expand(cluster_name_)),
              has_sharding_key(sharding_key_),
              sharding_key_expr(sharding_key_ ? buildShardingKeyExpression(sharding_key_, global_context,
    ...
    StorageDistributed::StorageDistributed(
            ...
            const String &data_path_,
            bool attach)
            : StorageDistributed(database_name, table_name_, columns_, String{}, String{}, cluster_name_, context_,
                                 sharding_key_, data_path_, attach) {
    ...
    StoragePtr StorageDistributed::createWithOwnCluster(
        ...
        auto res = ext::shared_ptr_helper<StorageDistributed>::create(
                String{}, table_name_, columns_, remote_database_, remote_table_, String{}, context_, ASTPtr(),
                String(), false);
    ...
    StoragePtr StorageDistributed::createWithOwnCluster(
            ...
            const Context &context_) {
        auto res = ext::shared_ptr_helper<StorageDistributed>::create(
                String{}, table_name_, columns_, remote_table_function_ptr_, String{}, context_, ASTPtr(), String(),
                false);
    ...
    StorageDistributed::getQueryProcessingStage(const Context &context, const ClusterPtr &cluster) const {
    ...
    BlockInputStreams StorageDistributed::read(
        ...
        Block header = materializeBlock(
                InterpreterSelectQuery(query_info.query, context,
                                       SelectQueryOptions(processed_stage)).getSampleBlock());

        ClusterProxy::SelectStreamFactory select_stream_factory = remote_table_function_ptr
                                                                  ? ClusterProxy::SelectStreamFactory(
                        header, processed_stage, remote_table_function_ptr, context.getExternalTables())
                                                                  : ClusterProxy::SelectStreamFactory(
                        header, processed_stage, QualifiedTableName{remote_database, remote_table},
    ...
    BlockOutputStreamPtr StorageDistributed::write(const ASTPtr &, const Context &context) {
        ...
        return std::make_shared<DistributedBlockOutputStream>(
                context, *this,
                createInsertToRemoteTableQuery(remote_database, remote_table, getSampleBlockNonMaterialized()), cluster,
                insert_sync, timeout);
    }
    ...
    void StorageDistributed::ClusterNodeData::requireConnectionPool(const std::string &name,
                                                                    const StorageDistributed &storage) {
        if (!conneciton_pool)
            conneciton_pool = StorageDistributedDirectoryMonitor::createPool(name, storage);
    }

    void
    StorageDistributed::ClusterNodeData::requireDirectoryMonitor(const std::string &name, StorageDistributed &storage) {
...
/// using constraints from "WHERE" condition, otherwise returns `nullptr`
    ...
    void registerStorageDistributed(StorageFactory &factory) {
        factory.registerStorage("Distributed", [](const StorageFactory::Arguments &args) {
            ...
            if (sharding_key) {
                auto sharding_expr = buildShardingKeyExpression(sharding_key, args.context,
                                                                args.columns.getAllPhysical(), true);
                const Block &block = sharding_expr->getSampleBlock();

                if (block.columns() != 1)
                    throw Exception("Sharding expression must return exactly one column",
