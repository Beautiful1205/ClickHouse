...
/** Parse the escape sequence, which can be simple (one character after backslash) or more complex (multiple characters).
  ...
  */
/**解析转义序列, 它可以是简单的(反斜杠后的一个字符), 也可以是更复杂的(多个字符).
 * 假定光标位于`\`符号上
 */
    template<typename Vector>
                ...
                if (code_point <= 0x7F) {
                    ...
                    if (code_point >= 0xD800 && code_point <= 0xDBFF) {
                        ...
                        if (second_code_point >= 0xDC00 && second_code_point <= 0xDFFF) {
                            UInt32 full_code_point =
                                    0x10000 + (code_point - 0xD800) * 1024 + (second_code_point - 0xDC00);
...
/** If enable_sql_style_quoting == true,
  ...
  */
    ...
    void readCSVStringInto(Vector &s, ReadBuffer &buf, const FormatSettings::CSV &settings) {
        ...
        if ((settings.allow_single_quotes && maybe_quote == '\'') ||
            (settings.allow_double_quotes && maybe_quote == '"')) {
            ++buf.position();

            /// The quoted case. We are looking for the next quotation mark.
            while (!buf.eof()) {
                char *next_pos = reinterpret_cast<char *>(memchr(buf.position(), maybe_quote,
            ...
            while (!buf.eof()) {
                ...
                [&]() {
                    ...
                    for (; next_pos + 15 < buf.buffer().end(); next_pos += 16) {
                        ...
                        auto eq = _mm_or_si128(_mm_or_si128(_mm_cmpeq_epi8(bytes, rc), _mm_cmpeq_epi8(bytes, nc)),
                                               _mm_cmpeq_epi8(bytes, dc));
    ...
    template void readCSVStringInto<PaddedPODArray<UInt8>>(PaddedPODArray<UInt8> &s, ReadBuffer &buf,
    ...
    void skipJSONField(ReadBuffer &buf, const StringRef &name_of_field) {
        ...
        } else if (isNumericASCII(*buf.position()) || *buf.position() == '-' || *buf.position() == '+' ||
        ...
        } else if (*buf.position() == '{') /// skip whole object
        {
            ...
            throw Exception(
                    "Unexpected symbol '" + std::string(*buf.position(), 1) + "' for key '" + name_of_field.toString() +
                    "'", ErrorCodes::INCORRECT_DATA);
        }
    }
