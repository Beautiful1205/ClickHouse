  ...
  */
    ...
    public:
        ...
        /** What limitations and quotas should be checked.
          ...
          */
        /* 需要检查哪些限制和配额
         * LIMITS_CURRENT: 仅检查当前流读取的数据量的最大结果行/字节限制(使用BlockStreamProfileInfo进行检查).
           仅用于检查root streams;
         * LIMITS_TOTAL: 检查从leaf streams读取的数据总量(即从磁盘和远程服务器读取的数据).
           它检查最大行数/字节数到正在读取的处理程序, 并为此使用ProcessListElement::progress中的信息.
           仅用于检查leaf streams;
        */
        enum LimitsMode {
            LIMITS_CURRENT,
    ...
    protected:
        ...
        std::shared_mutex children_mutex;//读写锁, 同时只能有一个写者或多个读者, 不能同时既有读者又有写者
    ...
    private:
        ...
        ///已经超过了行/字节数的限制, 需要在下一次“read”调用时停止执行, 就好像线程已用完一样
