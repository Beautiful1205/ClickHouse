...
namespace DB {
    ...
    NativeBlockInputStream::NativeBlockInputStream(ReadBuffer &istr_, const Block &header_, UInt64 server_revision_,
                                                   bool convert_types_to_low_cardinality_)
            : istr(istr_), header(header_), server_revision(server_revision_),
              convert_types_to_low_cardinality(convert_types_to_low_cardinality_) {
    }
    ...
    void NativeBlockInputStream::readData(const IDataType &type, IColumn &column, ReadBuffer &istr, size_t rows,
        ...
        type.deserializeBinaryBulkStatePrefix(settings, state);//反序列化二进制文件状态前缀
        type.deserializeBinaryBulkWithMultipleStreams(column, rows, settings, state);//使用多个流反序列化二进制数据. 读取不多于rows行的数据

        if (column.size() != rows)
            throw Exception("Cannot read all data in NativeBlockInputStream. Rows read: " + toString(column.size()) +
                            ". Rows expected: " + toString(rows) + ".",
    ...
    Block NativeBlockInputStream::readImpl() {
        ...
        for (size_t i = 0; i < columns; ++i) {
            if (use_index) {
                ...
                istr_concrete->seek(index_column_it->location.offset_in_compressed_file,
            ...
            /// Name
            readBinary(column.name, istr);//列名

            /// Type
            String type_name;
            readBinary(type_name, istr);  //列的类型
            column.type = data_type_factory.get(type_name);//根据列的类型名称获取到数据类型指针

            if (use_index) {
                /// Index allows to do more checks.
                if (index_column_it->name != column.name)
                    throw Exception("Index points to column with wrong name: corrupted index or data",
                                    ErrorCodes::INCORRECT_INDEX);
                if (index_column_it->type != type_name)
                    throw Exception("Index points to column with wrong type: corrupted index or data",
                                    ErrorCodes::INCORRECT_INDEX);
            }

            /// Data
            MutableColumnPtr read_column = column.type->createColumn();//数据类型指针调用createColumn()生成列指针

            double avg_value_size_hint = avg_value_size_hints.empty() ? 0 : avg_value_size_hints[i];
            if (rows)    /// If no rows, nothing to read.
                readData(*column.type, *read_column, istr, rows, avg_value_size_hint);//读取数据的方法, 具体实现

            column.column = std::move(read_column);//转换成右值引用

            /// Support insert from old clients without low cardinality type.
            bool revision_without_low_cardinality =
            ...
            if (header && (convert_types_to_low_cardinality || revision_without_low_cardinality)) {
                column.column = recursiveLowCardinalityConversion(column.column, column.type, header.getByPosition(i).type);//递归低基数转换
