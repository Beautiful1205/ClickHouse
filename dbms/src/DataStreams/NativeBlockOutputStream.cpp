...
namespace DB {
    ...
    void
    NativeBlockOutputStream::writeData(const IDataType &type, const ColumnPtr &column, WriteBuffer &ostr, UInt64 offset,
                                       UInt64 limit) {
    ...
    void NativeBlockOutputStream::write(const Block &block) {
        ...
        /// Dimensions
        size_t columns = block.columns();//block中的数据共有几列
        size_t rows = block.rows();      //block中的数据共有几行

        //把columns和rows写到ostr中
        writeVarUInt(columns, ostr);
        writeVarUInt(rows, ostr);

        /** The index has the same structure as the data stream.
          * But instead of column values, it contains a mark
          * that points to the location in the data file where this part of the column is located.
          */
        if (index_ostr) {
        ...
        for (size_t i = 0; i < columns; ++i) {
            ...
            /// 需要移除low cardinality类型  或者  向没有low cardinality 类型的历史版本client发送数据
            if (remove_low_cardinality ||
                (client_revision && client_revision < DBMS_MIN_REVISION_WITH_LOW_CARDINALITY_TYPE)) {
