...
//只要在第一次调用`read()` 或者 `readPrefix()`方法之前调用该类中的其他方法, 就可以调用不带互斥锁的子对象而不必担心安全问题.
    ...
    Block IBlockInputStream::read() {
        ...
        if (!checkTimeLimit())//检查是否超时/超出行数限制/超出字节数限制
            limit_exceeded_need_break = true;

        if (!limit_exceeded_need_break)//没有超出行/字节数的限制, 可以读取数据, 并将这些数据存在Block中
            //readImpl()方法是读取数据的具体实现
            res = readImpl();

        if (res) {//res不为空
            info.update(res);//更新BlockStreamProfileInfo, 用于分析的信息
            ...
            /** If the thread is over, then we will ask all children to abort the execution.
              ...
              */
            /* 如果线程结束, 那么我们将要求所有子线程中止执行. 这样做在SQL包含LIMIT子句的时候是有意义的
             *
             * 存在这样一种情况, 当所有必要的数据都已被读取后, 因为读取子类流是在单独的线程(甚至是远程)中工作的,
             * 所以需要执行cancel(false);取消子类流的执行
             */
            cancel(false);
        }
    ...
    bool IBlockInputStream::checkTimeLimit() {
        if (limits.max_execution_time != 0
            ...
            return handleOverflowMode(limits.timeout_overflow_mode,
                                      ...
                                      + " seconds, maximum: " +
                                      toString(limits.max_execution_time.totalMicroseconds() / 1000000.0),
    ...
    void IBlockInputStream::checkQuota(Block &block) {
        switch (limits.mode) {
            ...
            case LIMITS_CURRENT: {
                ...
                quota->checkAndAddExecutionTime(current_time,
                                                Poco::Timespan((total_elapsed - prev_elapsed) * 1000000.0));
    ...
    static void
    limitProgressingSpeed(size_t total_progress_size, size_t max_speed_in_seconds, UInt64 total_elapsed_microseconds) {
    ...
    void IBlockInputStream::progressImpl(const Progress &value) {
        ...
        if (process_list_elem) {
                    ...
                    case OverflowMode::THROW: {
                        if (limits.size_limits.max_rows && total_rows_estimate > limits.size_limits.max_rows)
                            throw Exception("Limit for rows to read exceeded: " + toString(total_rows_estimate)
                                            ...
                                            toString(limits.size_limits.max_rows),
                                            ErrorCodes::TOO_MANY_ROWS);
                        else
                            throw Exception(
                                    "Limit for (uncompressed) bytes to read exceeded: " + toString(progress.read_bytes)
            ...
            if (last_profile_events_update_time + profile_events_update_period_microseconds <
            ...
            if ((limits.min_execution_speed || limits.max_execution_speed || limits.min_execution_speed_bytes ||
                 ...
                 (total_rows && limits.timeout_before_checking_execution_speed != 0)) &&
                (static_cast<Int64>(total_elapsed_microseconds) >
                 limits.timeout_before_checking_execution_speed.totalMicroseconds())) {
                ...
                double elapsed_seconds = (throttler_sleep_microseconds > total_elapsed_microseconds)
                                         ? 0.0 : (total_elapsed_microseconds - throttler_sleep_microseconds) /
                                                 1000000.0;
                ...
                if (elapsed_seconds > 0) {
                    ...
                    if (limits.min_execution_speed_bytes &&
                        progress.read_bytes / elapsed_seconds < limits.min_execution_speed_bytes)
                        throw Exception(
                                "Query is executing too slow: " + toString(progress.read_bytes / elapsed_seconds)
                    ...
                    if (limits.max_execution_time != 0 && total_rows) {
                        double estimated_execution_time_seconds =
                                elapsed_seconds * (static_cast<double>(total_rows) / progress.read_rows);
                    ...
                    if (limits.max_execution_speed &&
                        progress.read_rows / elapsed_seconds >= limits.max_execution_speed)
                        limitProgressingSpeed(progress.read_rows, limits.max_execution_speed,
                                              total_elapsed_microseconds);

                    if (limits.max_execution_speed_bytes &&
                        progress.read_bytes / elapsed_seconds >= limits.max_execution_speed_bytes)
    ...
    size_t IBlockInputStream::checkDepthImpl(size_t max_depth, size_t level) const {
        ...
        if (level > max_depth)
            throw Exception("Query pipeline is too deep. Maximum: " + toString(max_depth),
    ...
    void IBlockInputStream::dumpTree(std::ostream &ostr, size_t indent, size_t multiplier) const {
        ...
        for (const auto &child : children) {
            ...
            /// Already printed subtrees are marked with zero in the array of multipliers.
