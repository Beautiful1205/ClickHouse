...
namespace DB {
    ...
    Block AggregatingBlockInputStream::readImpl() {
        if (!executed) {
            ...
            if (!aggregator.hasTemporaryFiles()) {//不包含临时文件的情况
                ...
                /** If there are temporary files with partially-aggregated data on the disk,
                  ...
                  *
                  *  将磁盘上包含中间聚合结果的临时文件读取到内存的Block中, 并合并它们, 以占用最少的内存
                  */
                ...
                impl = std::make_unique<MergingAggregatedMemoryEfficientBlockInputStream>(input_streams, params, final,
                                                                                          1, 1);
            }
        }
