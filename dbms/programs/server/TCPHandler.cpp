...
namespace DB {
        ...
        while (1) {
            ...
            try {
                ...
                if (client_revision >= DBMS_MIN_REVISION_WITH_SERVER_LOGS
                    ...
                    state.logs_queue->max_priority = Poco::Logger::parseLevel(
                            query_context->getSettingsRef().send_logs_level.toString());
    ...
    void TCPHandler::readData(const Settings &global_settings) {
        ...
        while (true) {
            ...
            if (!receivePacket())//调用receivePacket()方法
                break;
    ...
    void TCPHandler::processOrdinaryQuery() {
        ...
        if (state.io.in) {
            ...
            while (true) {
                ...
                while (true) {
                    if (isQueryCancelled()) {//查询是否被客户端取消
    ...
    bool TCPHandler::receivePacket() {
        ...
        switch (packet_type) {
            case Protocol::Client::Query:
                if (!state.empty())
                    throw NetException("Unexpected packet Query received from client",
                                       ErrorCodes::UNEXPECTED_PACKET_FROM_CLIENT);
                receiveQuery();//这个过程应该是读取in中的内容，相关信息赋值给QueryState
                return true;
            ...
            case Protocol::Client::Hello:
                throw Exception("Unexpected packet " + String(Protocol::Client::toString(packet_type)) +
                                " received from client",
                                ErrorCodes::UNEXPECTED_PACKET_FROM_CLIENT);

            case Protocol::Client::TablesStatusRequest:
                if (!state.empty())
                    throw NetException("Unexpected packet TablesStatusRequest received from client",
                                       ErrorCodes::UNEXPECTED_PACKET_FROM_CLIENT);
    ...
    void TCPHandler::receiveQuery() {
        ...
        /// Client info
        {
            ...
            //根据是否是集群查询(分布式查询)设置不同的query_kind
        ...
        settings.deserialize(*in);//反序列化in中的内容
        ...
        state.timeout_setter = std::make_unique<TimeoutSetter>(socket(), settings.receive_timeout,
    ...
    void TCPHandler::initBlockOutput(const Block &block) {
        if (!state.block_out) {
            if (!state.maybe_compressed_out) {
                std::string method = Poco::toUpper(
                        query_context->getSettingsRef().network_compression_method.toString());
    ...
    bool TCPHandler::isQueryCancelled() {
        ...
        if (static_cast<ReadBufferFromPocoSocket &>(*in).poll(0)) {
            ...
            switch (packet_type) {
                case Protocol::Client::Cancel:
                    if (state.empty())
                        throw NetException("Unexpected packet Cancel received from client",
                                           ErrorCodes::UNEXPECTED_PACKET_FROM_CLIENT);
        ...
        try {
            ...
            if (!strcmp(e.what(), "Timeout")) {
                LOG_DEBUG(log, "Poco::Exception. Code: " << ErrorCodes::POCO_EXCEPTION << ", e.code() = " << e.code()
                                                         << ", e.displayText() = " << e.displayText() << ", e.what() = "
                                                         << e.what());
            } else
                throw;
        }
