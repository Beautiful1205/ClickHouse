...
namespace DB {
    ...
    // ClickHouse Server启动的main()方法
    // 主要过程包括: 解析参数配置, 初始化server, 再启动服务监听端口.
    int Server::main(const std::vector<std::string> & /*args*/) {
        ...
        SCOPE_EXIT({
                         ...
                         * It is important to do early, not in destructor of Context,
                         * because table engines could use Context on destroy.
                         */
                       LOG_INFO(log, "Shutting down storages.");
                       global_context->shutdown();
                       LOG_DEBUG(log, "Shutted down storages.");

                       /** Explicitly destroy Context.
                         * It is more convenient than in destructor of Server, because logger is still available.
        ...
        // 设置max_open_files参数
        {
            ...
            if (rlim.rlim_cur == rlim.rlim_max) {
                ...
                if (rc != 0)
                    ...
                    LOG_DEBUG(log,
                              "Set max number of file descriptors to " << rlim.rlim_cur << " (was " << old << ").");
            }
        }
        ...
        if (config().has("interserver_http_port") && config().has("interserver_https_port"))
            throw Exception("Both http and https interserver ports are specified",
        ...
        for (auto[host_tag, port_tag, scheme] : interserver_tags) {
            if (config().has(port_tag)) {
                ...
                if (this_host.empty()) {
                    ...
                    LOG_DEBUG(log,
                              "Configuration parameter '" + String(host_tag) +
                              "' doesn't exist or exists and empty. Will use '" + this_host
                              + "' as replica host.");
                }
                ...
                if (port < 0 || port > 0xFFFF)
                    throw Exception("Out of range '" + String(port_tag) + "': " + toString(port),
        ...
        if (config().has("interserver_http_credentials")) {
            ...
            if (user.empty())
                throw Exception("Configuration parameter interserver_http_credentials user can't be empty",
                                ErrorCodes::NO_ELEMENTS_IN_CONFIG);

            global_context->setInterserverCredentials(user, password);
        }

        if (config().has("macros"))//如果包含宏定义, 处理宏定义
        ...
        auto main_config_reloader = std::make_unique<ConfigReloader>(config_path,
                                                                     ...
                                                                     [&](ConfigurationPtr config) {
                                                                         ...
                                                                         global_context->setMacros(
                                                                                 std::make_unique<Macros>(*config,
                                                                                                          "macros"));
                                                                     },
                /* already_loaded = */ true);
        ...
        global_context->setConfigReloadCallback([&]() {
            main_config_reloader->reload();//加载config.xml配置
            users_config_reloader->reload();//加载users.xml配置, 此时应该是只加载users.xml
        });
        ...
        if (uncompressed_cache_size > max_cache_size) {
            ...
            LOG_INFO(log, "Uncompressed cache size was lowered to "
        ...
        if (mark_cache_size > max_cache_size) {
            ...
            LOG_INFO(log,
                     "Mark cache size was lowered to " << formatReadableSizeWithBinarySuffix(uncompressed_cache_size)
                                                       << " because the system has low amount of memory");
        }
        ...
        if (has_zookeeper && config().has("distributed_ddl")) {
            ...
            global_context->setDDLWorker(
                    std::make_unique<DDLWorker>(ddl_zookeeper_path, *global_context, &config(), "distributed_ddl"));
        }
...

        {
            ...
            std::vector<std::unique_ptr<Poco::Net::TCPServer>> servers;//HTTPServer是TCPServer的子类, 所以这里用的TCPServer这个父类
            ...
            auto make_socket_address = [&](const std::string &host, UInt16 port) {
                ...
                try {
                    ...
                    if (code == EAI_FAMILY
                        ...
                        LOG_ERROR(log,
                                  "Cannot resolve listen_host (" << host << "), error " << e.code() << ": "
                                                                 << e.message() << ". "
            ...
            auto socket_bind_listen = [&](auto &socket, const std::string &host, UInt16 port,
                                          [[maybe_unused]] bool secure = 0) {
                ...
                socket.bind(address, /* reuseAddress = */ true);
#else
                socket.bind(address, /* reuseAddress = */ true, /* reusePort = */
                            config().getBool("listen_reuse_port", false));
#endif
                ...
                try {
                    ...
                    if (config().has("tcp_port_secure")) {
                        ...
                        servers.emplace_back(std::make_unique<Poco::Net::TCPServer>(
                                ...
                                socket,
                                new Poco::Net::TCPServerParams));
                        LOG_INFO(log, "Listening for connections with secure native protocol (tcp_secure): " +
                                      address.toString());
#else
                    ...
                    if (servers.empty())
                        throw Exception("No 'tcp_port' and 'http_port' is specified in configuration file.",
                                        ErrorCodes::NO_ELEMENTS_IN_CONFIG);
                    ...
                    if (config().has("interserver_https_port")) {
                        ...
                        LOG_INFO(log, "Listening for secure replica communication (interserver) https://" +
                                      address.toString());
#else
                        throw Exception{"SSL support for TCP protocol is disabled because Poco library was built without NetSSL support.",
                                ErrorCodes::SUPPORT_IS_DISABLED};
                    ...
                    if (config().has("mysql_port")) {
                        Poco::Net::ServerSocket socket;
                        auto address = socket_bind_listen(socket, listen_host,
                                                          config().getInt("mysql_port"), /* secure = */ true);
                    ...
                    std::string message = "Listen [" + listen_host + "]:" + std::to_string(listen_port) + " failed: " +
                                          std::to_string(e.code()) + ": " + e.what() + ": " + e.message();
                    if (listen_try)
            ...
            main_config_reloader->start();//加载config.xml配置, 此时应该是只加载config.xml
            users_config_reloader->start();//加载users.xml配置
            ...
            SCOPE_EXIT({
                           ...
                           LOG_INFO(log,
                                    "Closed all listening sockets."
                                            << (current_connections ? " Waiting for " + toString(current_connections) +
                           ...
                           LOG_INFO(
                                   log, "Closed connections."
                                           << (current_connections ? " But " + toString(current_connections) +
                                                                     " remains."
