...
namespace DB
{
    ...
    private:
        ...
        int mainImpl()
        {
            ...
            // 如果满足以下条件之一, 则表示不是命令行交互模式, 设置is_interactive = false, 并启用批处理模式:
            ...
            if (is_interactive)
                showClientVersion();//交互模式下展示客户端版本
            ...
            // 命令行中的settings覆盖clickhouse-client.xml中的配置
            ...
            //向服务端Server发起连接, 连接请求被服务端对应端口的Handler捕获并处理
            connect();
            ...
            {//针对交互式模式的请求
                ...
                if (print_time_to_stderr)
                    throw Exception("time option could be specified only in non-interactive mode",
                                    ErrorCodes::BAD_ARGUMENTS);
                ...
                if (!history_file.empty())
                {
                    if (Poco::File(history_file).exists())
                    {
                        ...
                        if (res)
                            std::cerr << "Cannot read history from file " + history_file + ": " +
                                         errnoToString(ErrorCodes::CANNOT_READ_HISTORY);
#endif
        ...
        void connect()
        {
            ...
            if (is_interactive)
                std::cout << "Connecting to "
                          << (!connection_parameters.default_database.empty() ? "database " +
            ...
            //getServerVersion()发送数据包给服务端, 发起握手连接
        ...
        void loop()
        {
            ...
            while (char *line_ = readline(input.empty() ? prompt().c_str() : ":-] "))
            {
                ...
                bool ends_with_semicolon = line[ws - 1] == ';';//分号结尾
                bool ends_with_backslash = line[ws - 1] == '\\';//反斜杠结尾
                                             ...
                                             (!config().has("multiline") && !hasDataInSTDIN())))
                {
                    if (input != prev_input)
                    {
                        ...
                        if (!history_file.empty() && append_history(1, history_file.c_str()))
                            std::cerr << "Cannot append history to file " + history_file + ": " +
                                         errnoToString(ErrorCodes::CANNOT_APPEND_HISTORY);
#endif
        ...
        void writeProgress()
        {
            ...
            if (elapsed_ns)
                message << " ("
                        ...
                        << formatReadableSizeWithDecimalSuffix(progress.read_bytes * 1000000000.0 / elapsed_ns)
                        << "/s.) ";
            else
                message << ". ";

            written_progress_chars = message.count() - prefix_size - (increment % 8 == 7 ? 10
        ...
        void init(int argc, char **argv)
        {
            ...
            main_description.add_options()
                    ...
                    ("disable_suggestion,A",
                     "Disable loading suggestion data. Note that suggestion data is loaded asynchronously through a second connection to ClickHouse server. Also it is reasonable to disable suggestion if you want to paste a query with TAB characters. Shorthand option -A is for those who get used to mysql client.")
                    ("always_load_suggestion_data",
                     "Load suggestion data even if clickhouse-client is run in non-interactive mode. Used for testing.")
                    ...
                    ("max_client_network_bandwidth", po::value<int>(),
